...
**Will NOT touch:** UI components, discovery UX, availability model, engagement lifecycle.

**Done condition:** Accept race is guarded at DB level. Extend route validates all fields. Relist rejects completed/cancelled dayworks. Templates validate currency. Onboarding validates avatar URLs.

---

#### 1. Accept race condition — DB-level guard in `apply_projection`

The bug: two concurrent accepts on a single-position daywork both pass the API-layer stale check. Both events are appended. The projection increments `positions_filled` twice — the second increment creates a phantom count with no actual engagement (because the first accept already rejected the other crew's application).

- [ ] New migration: update `apply_projection` DAYWORK.ACCEPTED handler
  - Before incrementing `positions_filled`, add a pre-check:
    ```sql
    -- Check positions not already full BEFORE incrementing
    ...
    if v_filled >= v_available then
      raise notice 'positions already full for daywork %, skipping accept', v_daywork_id;
      return;
    end if;
    ```
  - This causes the second concurrent accept to no-op at the projection level (event is still in the ledger for audit, but no state change occurs)
- [ ] Corresponding rollback file with previous `apply_projection` body
- [ ] Integration test: append two DAYWORK.ACCEPTED events for the same single-position daywork — verify `positions_filled = 1`, only one engagement created

---

#### 2. Extend route validation — `apps/web/src/app/api/daywork/[id]/extend/route.ts`

- [ ] Add check: `newEndDate >= daywork.end_date` (cannot "extend" backwards)
- [ ] Add `workingDays` bounds check: must be positive integer, must not exceed new date span
- [ ] Add `workingDayDates` validation if provided:
  - All dates must be valid YYYY-MM-DD
  - All dates must be within `[daywork.start_date, newEndDate]`
  - No duplicates
  - Length must match `workingDays` if both provided
- [ ] Add test for backward extension → 400
- [ ] Add test for invalid workingDayDates → 400

---

#### 3. Relist-with-dates status check — `apps/web/src/app/api/engagements/[id]/relist-with-dates/route.ts`

- [ ] Change status check from `if (daywork.status === 'active')` to:
  ```typescript
  if (daywork.status !== 'in_progress') {
    return NextResponse.json({ error: 'Daywork is not available for relisting' }, { status: 400 });
  }
  ```
  This blocks relist on `active` (already relisted), `completed`, and `cancelled`
- [ ] Add test for relist on completed daywork → 400
- [ ] Add test for relist on cancelled daywork → 400

---

#### 4. Template currency validation — `apps/web/src/app/api/daywork/templates/route.ts`

- [ ] In POST handler, validate `currency` against `['EUR', 'USD', 'GBP', 'AED']`:
  ```typescript
  ...
  ```
- [ ] Same validation in PATCH/PUT handler if one exists
- [ ] Add test for invalid currency → 400

---

#### 5. Onboarding avatar URL validation — `apps/web/src/app/api/onboarding/route.ts`

- [ ] Validate `avatarUrl` is a well-formed HTTPS URL if provided:
  ```typescript
  ...
  ```
- [ ] Add test for javascript: URL → 400
- [ ] Add test for valid HTTPS URL → passes
...
- [ ] Update `BUILD_STATE.md`:
  - Stage entry: `[Stage 110] API validation hardening — accept race guard in apply_projection, extend route full validation, relist status check, template currency validation, onboarding avatar URL validation`
  - Update schema version if migration added
  - Add migration to migration table
- [ ] Add to launch-readiness.md: new P1 row for accept race condition, mark `[x]`
