...
- [ ] Apply the same 5-minute cooldown per conversation for WhatsApp as exists for email (prevents spam on rapid MESSAGE.SENT events).
- [ ] Skip WhatsApp for system messages (`MESSAGE.SENT` with `is_system: true`) — same as existing push skip on line 79.
- [ ] Tests: mock `fetch` for Twilio API calls (no Twilio SDK to mock), verify template name mapping for all 26 event types, verify fallback chain (WhatsApp fail → push → email), verify no duplicate (WhatsApp success = push + email skipped), verify cooldown, verify system message skip
...
- [ ] Migration: extend `apply_projection` PERSON.DATA_SCRUBBED handler — add `DELETE FROM notification_channels WHERE person_id = p_person_id;` This is a hard delete of the encrypted phone. Must go BEFORE the existing profile anonymisation (no FK dependency, but logically scrub channels first).
- [ ] Rollback: must contain the full previous `apply_projection` body (self-contained per CLAUDE.md rule 4).
- [ ] Extend GDPR export (`/api/account/export`): query `notification_channels` for the requesting user using service client. If row exists and verified, decrypt the phone number and include in export as `whatsapp_phone: "+33612345678"`. If no row, omit the field. The export already uses `select('*')` on `user_preferences` so `whatsapp_enabled` is automatically included.
