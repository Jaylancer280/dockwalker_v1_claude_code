...
import { POST } from '@/app/api/daywork/[id]/applicants/[crewId]/shortlist/route';
...
describe('POST /api/daywork/:id/applicants/:crewId/shortlist', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('returns 400 when application already shortlisted', async () => {
    mockRequireDomainUser.mockResolvedValue(guardOk());
    mockFromAuth
      .mockReturnValueOnce(
        makeChain({ id: 'd1', poster_person_id: 'u1', status: 'active' }),
      )
      .mockReturnValueOnce(makeChain({ id: 'app1', status: 'shortlisted' }));

    const res = await POST(new Request('http://localhost'), makeParams('d1', 'c1'));
    expect(res.status).toBe(400);
  });

  it('returns 200 on successful shortlist from applied', async () => {
    mockRequireDomainUser.mockResolvedValue(guardOk());
    mockFromAuth
      .mockReturnValueOnce(
        makeChain({ id: 'd1', poster_person_id: 'u1', status: 'active' }),
    ...
    expect(mockRpc).toHaveBeenCalledWith(
      'append_event',
      expect.objectContaining({ p_event_type: 'DAYWORK.SHORTLISTED' }),
    );
  });

  it('returns 200 on successful shortlist from viewed', async () => {
    mockRequireDomainUser.mockResolvedValue(guardOk());
    mockFromAuth
      .mockReturnValueOnce(
        makeChain({ id: 'd1', poster_person_id: 'u1', status: 'active' }),
