...
- Notifications count (`GET /api/notifications/count`): hat-scoped unread counts for badge polling. Returns `{ notification_count, message_count, alt_notification_count, alt_message_count }`. Notifications are scoped by `role_context` column (crew/employer). Messages are scoped by the user's role in each engagement (crew_person_id vs employer_person_id). `NotificationBell` uses `notification_count`, `BottomNav` messages badge uses `message_count`, `HatSwitcher` shows `alt_notification_count + alt_message_count` as a combined badge for the other hat.
