...
- Crew experiences (`GET/POST /api/experiences`, `PATCH/DELETE /api/experiences/[id]`): CRUD for crew experience history. Salary fields stored server-side but NEVER returned in API responses. POST validates required fields (vesselId, roleId, startDate, charterOrPrivate), charter/private, rotation type, description length. PATCH/DELETE verify ownership.
