...
- Forgot password (`/auth/forgot-password`): email input → `supabase.auth.resetPasswordForEmail()` → success message. Auth callback (`/auth/callback`): handles both PKCE `code` exchange and `token_hash`+`type` fallback (for cross-browser/device email opens). Reset password (`/auth/reset-password`): reached via callback redirect → `onAuthStateChange` detects `PASSWORD_RECOVERY` event + `getSession()` fallback → new password form → `supabase.auth.updateUser()`. Both are public routes (no auth required). Production SMTP is configured in Supabase Dashboard, not in code — see `.env.example` for setup notes.
