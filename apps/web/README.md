...
- Vessel update (`PATCH /api/vessels/[id]`): updates vessel fields (name, vesselType, vesselOperation, loaMeters, ndaFlag). LOA changes auto-derive size band. NDA immutability guard: returns 400 if trying to set `ndaFlag: false` when current value is `true`. Owner-only.
