...
- Admin API: `is_admin` flag on persons, `requireAdmin` guard. Routes: `GET /api/admin/users` (search+paginate), `GET /api/admin/users/:id` (detail), `GET /api/admin/engagements` (stuck engagement finder), `POST /api/admin/engagements/:id/complete` (force-complete with reason), `GET/POST/PATCH /api/admin/canonical/:table` (canonical data CRUD). All admin actions append `ADMIN.*` audit events.
