...
const DEPARTMENT_COLORS: Record<string, string> = {
  deck: 'rgba(59,130,246,0.18)', // blue
  interior: 'rgba(168,85,247,0.18)', // purple
  galley: 'rgba(234,179,8,0.18)', // amber
  engineering: 'rgba(34,197,94,0.18)', // green
};

export function getDepartmentGradient(
  ...
  // Vary angle between 120-180deg based on seed for per-card visual variety
  const angle = seed ? 120 + (hashSeed(seed) % 61) : 135;
  return `linear-gradient(${angle}deg, ${color} 0%, transparent 80%)`;
}
