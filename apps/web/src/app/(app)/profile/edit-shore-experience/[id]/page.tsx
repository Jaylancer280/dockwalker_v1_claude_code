...
export default function EditShoreExperiencePage() {
  ...
  useEffect(() => {
    Promise.all([
      safeFetch<{ categories?: Category[] }>('/api/shore-experience-categories'),
      safeFetch<{ experiences?: ShoreExp[] }>('/api/shore-experiences'),
    ]).then(([catRes, expRes]) => {
      if (catRes.ok) setCategories(catRes.data.categories ?? []);
      if (expRes.ok) {
        const exp = (expRes.data.experiences ?? []).find((e) => e.id === (params.id as string));
        if (exp) {
          setCategoryId(exp.category_id);
          setEmployerName(exp.employer_name);
  ...
  async function handleSubmit() {
    ...
    const result = await safeFetch<{ error?: string }>(
      `/api/shore-experiences/${params.id as string}`,
      {
        method: 'PATCH',
    ...
    if (result.ok) {
      showSuccess('Shore experience updated');
      router.push('/profile');
    } else {
    ...
    <main className="flex min-h-svh flex-col bg-background">
      <header className="sticky top-0 z-10 border-b border-[var(--border)] bg-[var(--surface)] px-4 py-3">
        <div className="page-width flex items-center gap-3">
          ...
          <h1 className="text-lg font-semibold">Edit Shore-Based Experience</h1>
        </div>
      </header>
