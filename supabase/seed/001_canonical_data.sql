...
--
-- Historically seeded here. As of migration 00104 (Locations V1), the
-- canonical region/city/port data is created entirely by that migration's
-- pre-step UPSERT (which also preserves the original 55 port + 30 city
-- UUIDs for FK integrity with historical profile/daywork/experience rows)
-- and then extended with ~6K OSM-sourced marinas. Re-seeding the legacy
-- 7-region baseline here would conflict with the post-00104 state (regions
-- renamed + replaced by UUIDv5 country rows, cities re-parented). Left
-- intentionally empty.
--
-- Everything that previously lived in this block is now in
-- `supabase/migrations/00104_marinas_v1_expansion.sql`.

-- [legacy seed block moved to migration 00104 — see above]
