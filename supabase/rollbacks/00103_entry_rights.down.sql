-- Rollback for 00103_entry_rights.sql — SELF-CONTAINED.
--
-- Reverses: renames entry_rights → visa_types, swaps category→region, restores
-- the 5 original names on the preserved rows, deletes the 19 new rows, restores
-- the 5 deleted rows (with fresh UUIDs — originals were gen_random_uuid() at
-- seed time and cannot be recovered), renames profiles.entry_right_ids →
-- visa_ids, restores the RLS policy name, and replaces apply_projection with
-- the pre-migration body (copied verbatim from migration 00099).
--
-- Known one-way data loss:
--   * profile.visa_ids arrays that had deleted visa UUIDs stripped in forward
--     step 1 cannot be recovered
--   * freshly regenerated UUIDs for the 5 restored rows are NOT the originals

drop index if exists public.idx_entry_rights_category_sort;

drop policy if exists "Authenticated users can read entry_rights" on public.entry_rights;

alter table public.profiles rename column entry_right_ids to visa_ids;
alter table public.entry_rights rename to visa_types;

delete from public.visa_types where id in (
...
update public.visa_types set name = 'Schengen'             where name = 'Schengen visa';
update public.visa_types set name = 'B1/B2 (US)'           where name = 'US B1/B2';
update public.visa_types set name = 'C1/D (US)'            where name = 'US C1/D';
update public.visa_types set name = 'Australian Work Visa' where name = 'Australian MCV';

alter table public.visa_types drop column if exists category;
alter table public.visa_types add column if not exists region text;

update public.visa_types set region = 'Europe'       where name = 'Schengen';
update public.visa_types set region = 'Americas'     where name = 'B1/B2 (US)';
update public.visa_types set region = 'Americas'     where name = 'C1/D (US)';
update public.visa_types set region = 'Middle East'  where name = 'UAE Residence Visa';
update public.visa_types set region = 'Asia Pacific' where name = 'Australian Work Visa';
...
-- Restore apply_projection to the pre-00103 body (copied verbatim from
-- migration 00099). Column references revert to visa_ids.
