-- Rollback 00097: Remove admin blocking infrastructure
-- Self-contained: restores full apply_projection from 00096

-- 1. Drop new columns
alter table public.persons drop column if exists blocked_at;
alter table public.persons drop column if exists last_event_at;

-- 2. Revert cancelled_by CHECK (remove 'admin')
-- First clean up any admin-cancelled engagements that would violate the old CHECK
update public.active_engagements set cancelled_by = 'employer' where cancelled_by = 'admin';
...
alter table public.active_engagements add constraint active_engagements_cancelled_by_check
  check (cancelled_by in ('crew', 'employer', 'postponement'));

-- 3. Clean up any events with new admin types before restoring old projection
delete from public.events where event_type in ('ADMIN.USER_BLOCKED', 'ADMIN.USER_UNBLOCKED', 'ADMIN.ENGAGEMENT_CANCELLED', 'ADMIN.POSTING_HIDDEN');

-- 4. Restore apply_projection from 00096 (59 handlers, no admin blocking handlers, no last_event_at)
