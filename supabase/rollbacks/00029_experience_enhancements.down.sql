-- =============================================================================
-- Rollback 00029: Revert experience enhancements
-- =============================================================================

-- 1. Revert contract_type values back to rotation_type values
alter table public.crew_experiences drop constraint crew_experiences_contract_type_check;
-- Can't perfectly reverse the value migration, but set to NULL for safety
update public.crew_experiences set contract_type = null;

-- 2. Rename contract_type -> rotation_type, contract_details -> rotation_details
alter table public.crew_experiences rename column contract_type to rotation_type;
alter table public.crew_experiences rename column contract_details to rotation_details;

-- Re-add old rotation_type CHECK
alter table public.crew_experiences add constraint crew_experiences_rotation_type_check
  check (rotation_type in ('2:2', '3:1', '3:3', '5:1', 'permanent', 'seasonal', 'mlc_standard', 'other'));

-- 3. Rename vessel_operation -> charter_or_private on crew_experiences
alter table public.crew_experiences drop constraint crew_experiences_vessel_operation_check;
alter table public.crew_experiences rename column vessel_operation to charter_or_private;
alter table public.crew_experiences add constraint crew_experiences_charter_or_private_check
  check (charter_or_private in ('charter', 'private'));

-- 4. Drop vessel_type (motor/sail) from vessels
alter table public.vessels drop column vessel_type;

-- 5. Rename vessel_operation back to vessel_type on vessels
alter table public.vessels drop constraint vessels_vessel_operation_check;
alter table public.vessels rename column vessel_operation to vessel_type;
alter table public.vessels add constraint vessels_vessel_type_check
  check (vessel_type in ('private', 'charter'));

-- 6. Restore get_vessel_public without vessel_operation
...
-- 7. Restore apply_projection from migration 00028
