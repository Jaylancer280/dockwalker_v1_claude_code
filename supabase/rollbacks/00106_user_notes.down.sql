-- =============================================================================
-- Rollback 00106: drop user_notes, restore admin_delete_person + apply_projection
--
-- Order matters: drop the table LAST so apply_projection's old body (which no
-- longer references user_notes) is in place before anything else runs.
-- Actually simpler: drop the table first, then recreate prior versions of
-- the two functions. The old bodies don't reference user_notes, so there's
-- no ordering problem.
-- =============================================================================

drop table if exists public.user_notes;

-- Restore admin_delete_person to its 00099 body (no user_notes cleanup).
...
-- Restore apply_projection to its 00103 body (no user_notes reference in
-- PERSON.DATA_SCRUBBED). 68 handlers, same as before.
