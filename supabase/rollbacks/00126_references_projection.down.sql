-- Rollback for 00126_references_projection.sql
--
-- Audit P0-1 fix (2026-04-30): restores apply_projection to its 00123
-- form (no REFERENCE.* handlers, no references-aware extensions to
-- PROFILE.CREATED / EXPERIENCE.UPDATED / EXPERIENCE.REMOVED /
-- PERSON.DATA_SCRUBBED). Pairs with 00125's schema rollback which drops
-- the references + reference_contacts tables — running this rollback
-- before 00125 leaves the references handlers in place but with
-- still-existing tables (no error); rolling back 00125 first then this
-- one would leave broken handlers, so the standard reverse order
-- (00126 → 00125) keeps schema and function aligned.
