-- Rollback 00025: Remove port_id from availability_windows
-- Restore apply_projection to migration 00024 version (without port_id)

drop index if exists public.idx_availability_port;

alter table public.availability_windows drop column if exists port_id;

-- Restore apply_projection from migration 00024 (city_id but no port_id)
  ...
  case p_event_type
    ...
    when 'AVAILABILITY.SET' then
      if coalesce((p_payload->>'not_available')::boolean, false) then
        ...
        values (p_person_id, current_date, (p_payload->>'expires_at')::timestamptz, (p_payload->>'city_id')::uuid, true)
        on conflict (person_id, date) do update set expires_at = excluded.expires_at, city_id = excluded.city_id, not_available = true, created_at = now();
      else
        ...
        insert into public.availability_windows (person_id, date, expires_at, city_id, not_available)
        select p_person_id, d::date, (p_payload->>'expires_at')::timestamptz, (p_payload->>'city_id')::uuid, false
        from generate_series((p_payload->>'start_date')::date, (p_payload->>'end_date')::date, '1 day'::interval) d
        on conflict (person_id, date) do update set expires_at = excluded.expires_at, city_id = excluded.city_id, not_available = false, created_at = now();
      end if;
