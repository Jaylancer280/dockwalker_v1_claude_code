...
--
-- Audit P0-1 fix (2026-04-30): the prior NOTICE-only rollback left the
-- apply_projection function with VESSEL.RENAMED, VESSEL.REFLAGGED,
-- VESSEL.METADATA_UPDATED handlers and the VESSEL.CREATED extension
-- still in place — broken if 00120 rollback (which drops vessel_names /
-- vessel_flag_states) runs subsequently. Restores the function to its
-- 00116 form (PROFILE multi-nationality projection) line-for-line.
--
