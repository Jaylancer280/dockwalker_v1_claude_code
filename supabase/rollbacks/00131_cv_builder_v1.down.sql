...
--
-- Reverses everything 00131 added:
--   - applications.invited_from_id FK + index
--   - permanent_invitations table (CASCADE drops indexes, RLS, partial idx)
--   - references.include_on_cv column
--   - crew_experiences.cv_show_full_vessel column
--   - profiles cv_handle / cv_handle_updated_at / cv_include_sea_time / cv_generated_at
--   - 'permanent_invitation' aggregate_type CHECK value
--   - apply_projection extensions (CV.GENERATED, CV.HANDLE_REGENERATED,
--     PERMANENT.INVITED handlers + PERMANENT.APPLIED invited_from_id branch)
--
-- Audit P0-1 fix (2026-04-30): the prior version of this rollback was
-- NOTICE-only for apply_projection, which left the function referencing
-- the dropped CV columns / permanent_invitations table after the schema
-- rollback ran. Now restores the function body to its 00130 form
-- line-for-line (no CV / PERMANENT.INVITED handlers).
--
-- Order: schema drops first (matches forward convention), then function
-- restoration via CREATE OR REPLACE. The function is not CALLED during
-- rollback so order is also safe SQL-wise.
...
-- 7. apply_projection — restore prior body (00130 form, no CV / PERMANENT.INVITED
--    handlers). Inlined verbatim from migration 00130 per the audit P0-1 fix.
