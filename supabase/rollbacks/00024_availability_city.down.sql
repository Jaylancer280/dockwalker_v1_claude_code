-- Rollback 00024: Remove city_id and not_available from availability_windows
-- Revert apply_projection to previous version (from migration 00023)

-- Drop the index first
drop index if exists public.idx_availability_city;

-- Drop the columns
alter table public.availability_windows drop column if exists not_available;
alter table public.availability_windows drop column if exists city_id;

-- Restore apply_projection from migration 00023 (without city_id in AVAILABILITY.SET handler)
