-- Rollback for 00136_engagement_phase_projection.sql
--
-- Restores the apply_projection function body to its 00132 state, which was
-- the most recent prior rewrite. 00133-00135 did not touch apply_projection.
--
-- Effect of rollback: removes the PERMANENT.SHORTLIST_CHAT_OPENED handler,
-- removes the SELECTED-time cascade that closes other shortlist chats,
-- removes phase guards on engagement events, removes WITHDRAWN/REJECTED/
-- CANCELLED_BY_EMPLOYER cascade for shortlist chats. Body below is an exact
-- copy of 00132's apply_projection.
