...
-- Drop RLS policies on permanent_templates
drop policy if exists "Owner can read own permanent templates" on public.permanent_templates;
drop policy if exists "Owner can insert own permanent templates" on public.permanent_templates;
drop policy if exists "Owner can update own permanent templates" on public.permanent_templates;
drop policy if exists "Owner can delete own permanent templates" on public.permanent_templates;

-- Drop permanent-specific RLS policies on applications
drop policy if exists "Crew can read own permanent applications" on public.applications;
drop policy if exists "Employer can read applications on own permanent postings" on public.applications;

-- Drop XOR constraints
alter table public.applications drop constraint if exists applications_posting_xor;
alter table public.active_engagements drop constraint if exists engagements_posting_xor;

-- Drop partial unique indexes
drop index if exists public.applications_crew_daywork_unique;
drop index if exists public.applications_crew_permanent_unique;

-- Restore original unique constraint on applications
alter table public.applications add constraint applications_crew_person_id_daywork_id_key unique (crew_person_id, daywork_id);

-- Drop new columns from applications
alter table public.applications drop column if exists rejection_reason;
alter table public.applications drop column if exists permanent_posting_id;
drop index if exists idx_applications_permanent;

-- Restore daywork_id NOT NULL on applications
alter table public.applications alter column daywork_id set not null;
...
-- Drop new columns from active_engagements
drop index if exists idx_engagements_permanent;
alter table public.active_engagements drop column if exists outcome;
alter table public.active_engagements drop column if exists permanent_posting_id;

-- Restore daywork_id NOT NULL on active_engagements
alter table public.active_engagements alter column daywork_id set not null;

-- Restore active_engagements status check without 'closed'
...
alter table public.profiles drop column if exists currently_employed;
alter table public.profiles drop column if exists notice_period_days;
alter table public.profiles drop column if exists permanent_availability;

-- Drop permanent_templates table
drop table if exists public.permanent_templates;

-- Drop permanent_postings table
drop table if exists public.permanent_postings;
...
alter table public.events add constraint events_aggregate_type_check
  check (aggregate_type in ('person', 'vessel', 'daywork', 'application', 'message',
    'engagement', 'checklist', 'invitation', 'experience', 'admin'));

-- Restore apply_projection to the exact body from migration 00057
