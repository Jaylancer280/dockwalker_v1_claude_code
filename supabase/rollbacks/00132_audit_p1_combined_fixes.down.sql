-- Rollback for 00132_audit_p1_combined_fixes.sql
--
-- Reverses all five audit P1 fixes:
--   P1-D1 PERMANENT.INVITED state guard
--   P1-D3 PERMANENT.SHORTLISTED status tightening
--   P1-D4 permanent_invitations FK on delete restrict
--   P1-M1 + P1-M2 NDA reveal employer-side + completed/closed status
--   P1-P4 idx_persons_deactivated_at
--
-- Lessons-mandated invariant: this rollback is fully self-contained.
-- Restores apply_projection to the 00131 body line-for-line and
-- restores get_vessel_public + get_vessels_public_batch to the 00122
-- bodies. Rolling back 00132 alone leaves the database in the
-- pre-00132 state with no orphan references.

-- 1. Drop the persons.deactivated_at index.
drop index if exists public.idx_persons_deactivated_at;

-- 2. Restore the FK cascade on permanent_invitations.permanent_posting_id.
...
-- 3. Restore 00122 versions of get_vessel_public + get_vessels_public_batch
--    (no employer-side IMO reveal, ae.status = 'active' only).
...
-- 4. Restore 00131's apply_projection body (no PERMANENT.INVITED state
--    guard, PERMANENT.SHORTLISTED includes 'viewed' status). Inlined
--    verbatim from migration 00131.
