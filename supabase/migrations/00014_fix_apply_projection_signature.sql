-- =============================================================================
-- Migration 00014: Fix apply_projection function signature
-- =============================================================================
-- Migrations 00010-00013 accidentally created/updated a 0-arg trigger version
-- of apply_projection(), but append_event() calls the 6-arg standalone version.
-- This migration updates the 6-arg version with all logic from 00010-00013
-- (shortlist, in_progress, crew completion, ratings) and drops the orphaned
-- 0-arg trigger version.
-- =============================================================================

-- Drop the orphaned 0-arg trigger version (never called by anything)
drop function if exists public.apply_projection();

-- Add 'shortlisted' to applications status CHECK (was missing from migration 00010)
alter table public.applications drop constraint applications_status_check;
alter table public.applications add constraint applications_status_check
  check (status in ('applied', 'viewed', 'shortlisted', 'accepted', 'rejected', 'withdrawn', 'superseded', 'completed', 'cancelled_by_crew', 'cancelled_by_employer'));

-- Add 'engagement' to events aggregate_type CHECK (was missing from migration 00012)
...
-- Allow engagement participants to read the daywork (needed after status moves to in_progress)
create policy "Engagement participants can read their daywork"
  on public.dayworks for select
  using (
    exists (
      select 1 from public.active_engagements ae
      where ae.daywork_id = dayworks.id
      and (ae.crew_person_id = auth.uid() or ae.employer_person_id = auth.uid())
    )
  );

-- Replace the 6-arg version (called by append_event) with full logic
