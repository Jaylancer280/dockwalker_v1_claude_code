-- =============================================================================
-- Migration 00097: Admin blocking, last_event_at tracking, new admin event handlers
--
-- Schema changes:
--   - persons.blocked_at (timestamptz, nullable)
--   - persons.last_event_at (timestamptz, nullable)
--   - active_engagements.cancelled_by CHECK extended with 'admin'
--
-- New apply_projection handlers:
--   - ADMIN.USER_BLOCKED — sets persons.blocked_at
--   - ADMIN.USER_UNBLOCKED — clears persons.blocked_at
--   - ADMIN.ENGAGEMENT_CANCELLED — cancels engagement + posting + applications + system message
--   - ADMIN.POSTING_HIDDEN — cancels daywork/permanent posting
--
-- Additive change:
--   - last_event_at updated at end of every event where p_person_id is not null
--
-- Backfill:
--   - last_event_at populated from MAX(events.created_at) per person
-- =============================================================================
...
-- 2. Extend cancelled_by CHECK to include 'admin'
...
-- 3. Replace apply_projection with all existing handlers + new admin handlers + last_event_at
...
begin
  case p_event_type
    ...
    when 'EXPERIENCE.ADDED' then
      insert into public.crew_experiences (
        ...
        sea_time_days, sea_time_nautical_miles
      ) values (
        (p_payload->>'id')::uuid,
        p_person_id,
        ...
        (p_payload->>'sea_time_days')::int,
        (p_payload->>'sea_time_nautical_miles')::int
      );
    ...
    when 'EXPERIENCE.UPDATED' then
      update public.crew_experiences set
        ...
        sea_time_days = coalesce((p_payload->>'sea_time_days')::int, sea_time_days),
        sea_time_nautical_miles = coalesce((p_payload->>'sea_time_nautical_miles')::int, sea_time_nautical_miles),
        updated_at = now()
      where id = p_aggregate_id::uuid
    ...
    when 'DAYWORK.POSTED' then
      insert into public.dayworks (id, poster_person_id, role_context, vessel_id, role_id, location_port_id, start_date, end_date, working_days, required_certification_ids, experience_bracket_id, day_rate, currency, meals, notes, positions_available, permanent_opportunity, required_languages)
    ...
    when 'DAYWORK.INVITATION_ACCEPTED' then
      ...
      select positions_filled, positions_available into v_filled, v_available
        from public.dayworks where id = (p_payload->>'daywork_id')::uuid;
      if v_filled >= v_available then
        raise notice 'positions already full for daywork %, skipping invitation accept', p_payload->>'daywork_id';
        return;
      end if;
      insert into public.active_engagements (crew_person_id, employer_person_id, daywork_id, start_date, end_date)
      values (
        (p_payload->>'crew_person_id')::uuid,
        (p_payload->>'employer_person_id')::uuid,
        (p_payload->>'daywork_id')::uuid,
        (p_payload->>'start_date')::date,
        (p_payload->>'end_date')::date
      );
      ...
      update public.applications set status = 'superseded', updated_at = now()
        ...
        and status in ('applied', 'viewed', 'shortlisted')
        and daywork_id != (p_payload->>'daywork_id')::uuid
        and daywork_id in (
          select d2.id from public.dayworks d2
          join public.dayworks d1 on d1.id = (p_payload->>'daywork_id')::uuid
    ...
    when 'AVAILABILITY.SET' then
      if coalesce((p_payload->>'not_available')::boolean, false) then
        ...
        update public.availability_windows set expires_at = now() where person_id = p_person_id and expires_at > now();
        insert into public.availability_windows (person_id, date, expires_at, city_id, port_id, not_available)
        select p_person_id, d::date, d::date + interval '1 day', (p_payload->>'city_id')::uuid, (p_payload->>'port_id')::uuid, false
    ...
    when 'PERMANENT.POSTED' then
      insert into public.permanent_postings (
        ...
        required_languages,
        contract_type, contract_details, description, meals, positions_available
      ) values (
        (p_payload->>'id')::uuid,
        p_person_id,
        (p_payload->>'vessel_id')::uuid,
        ...
        p_payload->>'notes',
        coalesce((select array_agg(x::text) from jsonb_array_elements_text(p_payload->'required_languages') x), '{}'),
        p_payload->>'contract_type',
        p_payload->>'contract_details',
        p_payload->>'description',
        coalesce((select array_agg(x::text) from jsonb_array_elements_text(p_payload->'meals') x), '{}'),
        coalesce((p_payload->>'positions_available')::int, 1)
      );
    ...
    when 'PERMANENT.PLACEMENT_CONFIRMED' then
      ...
      update public.permanent_postings set status = 'filled', updated_at = now()
        where id = v_posting_id;
      update public.applications set status = 'placement_confirmed', updated_at = now()
        where permanent_posting_id = v_posting_id
        and status = 'selected';
  ...
  -- Track last activity for every event with a person_id
...
-- 4. Backfill last_event_at from existing events
update public.persons set last_event_at = (
  select max(created_at) from public.events where person_id = persons.id
);
