-- =============================================================================
-- Migration 00030: Daywork invitations
--
-- 1. Create daywork_invitations table
-- 2. Add RLS policies
-- 3. Add indexes
-- 4. Update apply_projection for invitation events + revocation logic
-- =============================================================================

-- -----------------------------------------------------------------------------
-- 1. Create daywork_invitations table
-- -----------------------------------------------------------------------------
create table public.daywork_invitations (
...
-- One invitation per crew per posting
alter table public.daywork_invitations
  add constraint daywork_invitations_unique_crew_daywork
  unique (daywork_id, crew_person_id);

-- -----------------------------------------------------------------------------
-- 2. RLS policies
-- -----------------------------------------------------------------------------
alter table public.daywork_invitations enable row level security;

-- Employer can read invitations they sent
create policy "employer_read_own_invitations" on public.daywork_invitations
  for select using (employer_person_id = auth.uid());

-- Crew can read invitations sent to them
create policy "crew_read_own_invitations" on public.daywork_invitations
  for select using (crew_person_id = auth.uid());

-- Writes via service role only (no insert/update/delete policies for authenticated users)

-- -----------------------------------------------------------------------------
-- 3. Indexes
-- -----------------------------------------------------------------------------
create index idx_invitations_daywork on public.daywork_invitations(daywork_id);
create index idx_invitations_crew on public.daywork_invitations(crew_person_id);
create index idx_invitations_status on public.daywork_invitations(status);
...
begin
  case p_event_type
    ...
    when 'DAYWORK.CANCELLED_BY_EMPLOYER' then
      ...
      -- Revoke all pending invitations for this daywork
    ...
    when 'DAYWORK.RELISTED' then
      ...
      -- Revoke all pending invitations (terms changed)
    ...
    when 'DAYWORK.APPLIED' then
      ...
      -- If crew was invited to this daywork, auto-accept the invitation
    ...
    when 'DAYWORK.ACCEPTED' then
      ...
      -- Revoke all pending invitations for this daywork (crew accepted, job filled)
