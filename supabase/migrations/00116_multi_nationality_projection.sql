-- Multi-nationality projection wiring (phase 2 of multi-nationality work).
--
-- Phase 1 (migration 00114) added the `profiles.nationality_ids uuid[]`
-- column and backfilled it from the legacy single `nationality_id` field.
-- This migration teaches `apply_projection` to write the array column on
-- PROFILE.CREATED + PROFILE.UPDATED events while preserving the legacy
-- single column as a denormalised "primary nationality" (= first element
-- of the array). Dual-write keeps existing reads working through the
-- transition window — a future migration can drop `nationality_id` once
-- every reader uses `nationality_ids`.
--
-- Payload backwards-compatibility: handlers accept either the new
-- `nationality_ids: uuid[]` field OR the legacy `nationality_id: uuid`
-- field. If both are present, the array wins. If only the legacy single
-- is present, it's wrapped into a one-element array.
--
-- The function body below is otherwise an exact copy of the latest
-- `apply_projection` definition (00108_admin_delete_full_wipe.sql) with
-- ONLY the PROFILE.CREATED and PROFILE.UPDATED handlers modified.
