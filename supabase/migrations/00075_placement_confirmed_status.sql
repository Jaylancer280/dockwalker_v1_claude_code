-- =============================================================================
-- Fix: Add placement_confirmed terminal status for permanent applications
-- =============================================================================
-- When PERMANENT.PLACEMENT_CONFIRMED fires, the selected application stays in
-- 'selected' status — it should move to 'placement_confirmed' so it no longer
-- appears as active in the Applied tab.
-- =============================================================================

-- 1. Expand the application status CHECK constraint
...
alter table public.applications add constraint applications_status_check
  check (status in ('applied', 'viewed', 'shortlisted', 'accepted', 'rejected',
    ...
    'selected', 'not_selected', 'placement_confirmed'));

-- 2. Replace apply_projection with the updated PERMANENT.PLACEMENT_CONFIRMED handler
-- The only change is adding: update selected application to placement_confirmed
  ...
  case p_event_type
    ...
    when 'PERMANENT.PLACEMENT_CONFIRMED' then
      ...
      update public.permanent_postings set status = 'filled', updated_at = now()
        where id = v_posting_id;
      -- Mark selected application as placement_confirmed (terminal)
