-- =============================================================================
-- Migration 00106: Admin user notes
--
-- Adds the `user_notes` table used by admins to record per-user moderation
-- context. Service-role only (no RLS policies for authenticated users).
--
-- - `person_id`   — note subject. ON DELETE RESTRICT (scrubbed via projection).
-- - `admin_person_id` — note author. ON DELETE SET NULL when an admin is
--                       scrubbed so notes survive with "unknown admin"
--                       attribution. Author-only edits enforced at the API
--                       layer.
--
-- Extends `PERSON.DATA_SCRUBBED` handler in `apply_projection` to rewrite the
-- content of notes ABOUT the scrubbed user to '[content scrubbed]' so the
-- audit trail survives without retaining free-text PII. Also extends
-- `admin_delete_person` RPC to clean up `user_notes` rows for both subject
-- and author.
-- =============================================================================
...
-- 2. RLS — enable with no authenticated policies (deny-all). Admin reaches
--    the table via service_role which bypasses RLS.
alter table public.user_notes enable row level security;

-- 3. Extend admin_delete_person to cover user_notes
...
-- 4. Replace apply_projection — same 68 handlers as 00103, PERSON.DATA_SCRUBBED
--    extended to rewrite user_notes content about the scrubbed subject.
