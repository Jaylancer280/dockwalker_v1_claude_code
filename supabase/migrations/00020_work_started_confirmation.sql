-- Migration 00020: Work started mutual confirmation
-- Adds work_started_status and work_started_at to active_engagements.
-- Either party can initiate "work started", the other confirms.
-- Once confirmed, postponement is blocked.

-- Add columns
alter table public.active_engagements
  add column if not exists work_started_status text
    check (work_started_status in ('initiated_by_crew', 'initiated_by_employer', 'confirmed')),
  add column if not exists work_started_at timestamptz;

-- Update apply_projection with new event handlers
