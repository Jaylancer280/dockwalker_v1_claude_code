...
--
-- Spec: tasks/country-entry-rights-visas.md
-- Strategy:
--   1. Strip the 5 deleted visa UUIDs from every profile's visa_ids array
--      (matched by name, so we don't need to know the auto-generated UUIDs at
--      migration-write time)
--   2. Rename visa_types → entry_rights, swap `region` for `category`
--   3. UPDATE the 5 preserved rows (rename + set category) by name
--   4. DELETE the 5 deleted rows by name
--   5. INSERT the 19 new canonical rows with hardcoded UUIDs
--   6. Rename profiles.visa_ids → entry_right_ids
--   7. Re-issue the read-only RLS policy with the new table name
--   8. Add an index on (category, sort_order) for picker queries
--   9. Replace apply_projection (full body) so PROFILE.CREATED and
--      PROFILE.UPDATED handlers coalesce the payload key (entry_right_ids
--      preferred, visa_ids fallback) and filter against live entry_rights IDs
--      so event replay can never reintroduce a dropped row.

-- =====================================================================
-- Step 1: Strip the 5 deleted visa UUIDs from live profiles.visa_ids.
-- =====================================================================

update public.profiles
set visa_ids = coalesce(
  (
    select array_agg(v)
...
-- =====================================================================
-- Step 2: Rename visa_types → entry_rights, drop region, add category.
-- =====================================================================

alter table public.visa_types rename to entry_rights;

alter table public.entry_rights drop column if exists region;

alter table public.entry_rights
  add column if not exists category text not null default 'visa'
    check (category in ('citizenship', 'residence', 'visa'));

-- =====================================================================
-- Step 3: UPDATE preserved rows (rename + set category) by name.
-- =====================================================================

update public.entry_rights set name = 'Schengen visa',       category = 'visa',      sort_order = 1  where name = 'Schengen';
update public.entry_rights set name = 'US B1/B2',            category = 'visa',      sort_order = 2  where name = 'B1/B2 (US)';
update public.entry_rights set name = 'US C1/D',             category = 'visa',      sort_order = 3  where name = 'C1/D (US)';
update public.entry_rights set name = 'Australian MCV',      category = 'visa',      sort_order = 6  where name = 'Australian Work Visa';
update public.entry_rights set name = 'UAE Residence Visa',  category = 'residence', sort_order = 4  where name = 'UAE Residence Visa';

-- =====================================================================
-- Step 4: DELETE the 5 deleted rows.
-- =====================================================================

delete from public.entry_rights
where name in (
  'UK Work Visa',
  'Canadian Work Permit',
  'Bahamian Work Permit',
...
-- =====================================================================
-- Step 5: INSERT 19 new canonical rows.
-- =====================================================================

insert into public.entry_rights (id, name, category, sort_order) values
  -- Citizenship (6)
  ('f2000000-0000-0000-0000-000000000001', 'EU/EEA/Swiss',                              'citizenship', 1),
  ('f2000000-0000-0000-0000-000000000002', 'UK',                                        'citizenship', 2),
  ('f2000000-0000-0000-0000-000000000003', 'US',                                        'citizenship', 3),
  ('f2000000-0000-0000-0000-000000000004', 'Canada',                                    'citizenship', 4),
  ('f2000000-0000-0000-0000-000000000005', 'Australia',                                 'citizenship', 5),
  ('f2000000-0000-0000-0000-000000000006', 'New Zealand',                               'citizenship', 6),
  -- Residence (7) — UAE Residence Visa preserved at sort_order=4
  ('f2000000-0000-0000-0000-000000000010', 'US Green Card (Lawful Permanent Resident)', 'residence', 1),
  ('f2000000-0000-0000-0000-000000000011', 'UK ILR (Indefinite Leave to Remain)',       'residence', 2),
  ('f2000000-0000-0000-0000-000000000012', 'EU long-term residence',                    'residence', 3),
  ('f2000000-0000-0000-0000-000000000013', 'Canadian PR (Permanent Resident)',          'residence', 5),
  ('f2000000-0000-0000-0000-000000000014', 'Australian PR (Permanent Resident)',        'residence', 6),
  ('f2000000-0000-0000-0000-000000000015', 'New Zealand PR (Permanent Resident)',       'residence', 7),
  -- Visa (11) — Schengen (1), US B1/B2 (2), US C1/D (3), Australian MCV (6) already preserved
  ('f2000000-0000-0000-0000-000000000020', 'US ESTA (Visa Waiver Program)',             'visa', 4),
  ('f2000000-0000-0000-0000-000000000021', 'UK Standard Visitor',                       'visa', 5),
  ('f2000000-0000-0000-0000-000000000022', 'Canadian eTA',                              'visa', 7),
  ('f2000000-0000-0000-0000-000000000023', 'NZ NZeTA',                                  'visa', 8),
  ('f2000000-0000-0000-0000-000000000024', 'Turkish İkamet',                            'visa', 9),
  ('f2000000-0000-0000-0000-000000000025', 'Bahamian visitor permit',                   'visa', 10),
  ('f2000000-0000-0000-0000-000000000026', 'Mexican FMM',                               'visa', 11);

-- =====================================================================
-- Step 6: Rename profiles.visa_ids → entry_right_ids.
-- =====================================================================

alter table public.profiles rename column visa_ids to entry_right_ids;

-- =====================================================================
-- Step 7: Re-issue the RLS policy with the renamed table.
-- =====================================================================

drop policy if exists "Authenticated users can read visa_types" on public.entry_rights;
create policy "Authenticated users can read entry_rights"
  on public.entry_rights for select
  to authenticated
  using (true);

-- =====================================================================
-- Step 8: Category+sort_order index for the picker.
-- =====================================================================

create index if not exists idx_entry_rights_category_sort
  on public.entry_rights (category, sort_order);

-- =====================================================================
-- Step 9: Replace apply_projection (full body from 00099) with the two
-- PROFILE handlers updated: column rename + filtered coalesce of
-- entry_right_ids | visa_ids against live entry_rights.id.
-- =====================================================================
  ...
  case p_event_type
    ...
    when 'PROFILE.CREATED' then
      insert into public.profiles (
        ...
        avatar_url, nationality_id, entry_right_ids,
