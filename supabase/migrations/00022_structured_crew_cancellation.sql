-- Migration 00022: Structured crew cancellation
-- Adds cancelled_by column to active_engagements to track initiator.
-- Expands cancellation_reason_category CHECK to include crew-specific reasons.
-- Updates apply_projection to write reason fields on crew cancellation.

-- ---------------------------------------------------------------------------
-- 1. Add cancelled_by column (crew | employer | postponement)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2. Expand cancellation_reason_category CHECK to include crew reasons
-- ---------------------------------------------------------------------------
alter table public.active_engagements
...
alter table public.active_engagements
  add constraint active_engagements_cancellation_reason_category_check
    check (cancellation_reason_category in (
      -- employer reasons
      'vessel_leaving', 'crew_requirements_changed', 'vessel_operational',
      -- crew reasons
      'personal_reasons', 'found_other_work', 'unsafe_conditions',
      -- shared
      'postponement', 'other'
    ));

-- ---------------------------------------------------------------------------
-- 3. Backfill cancelled_by for existing cancelled engagements
-- ---------------------------------------------------------------------------
update public.active_engagements
set cancelled_by = case
  when cancellation_reason_category = 'postponement' then 'postponement'
  when cancellation_reason_category is not null then 'employer'
  else null
end
where status = 'cancelled' and cancelled_by is null;

-- ---------------------------------------------------------------------------
-- 4. Update apply_projection with structured crew cancellation
-- ---------------------------------------------------------------------------
