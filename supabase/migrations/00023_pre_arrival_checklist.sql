-- Migration 00023: Pre-arrival checklist
-- Adds engagement_checklists table for employer-declared pre-arrival items.
-- Crew can acknowledge individual items via toggle.
-- New events: CHECKLIST.SET, CHECKLIST.ITEM_TOGGLED.

-- ---------------------------------------------------------------------------
-- 1. Add 'checklist' to events aggregate_type CHECK
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2. Create engagement_checklists table
-- ---------------------------------------------------------------------------
create table if not exists public.engagement_checklists (
  engagement_id uuid primary key references public.active_engagements(id),
  items jsonb not null default '[]'::jsonb,
  acknowledged_item_ids text[] not null default '{}',
...
-- 3. RLS policies
-- ---------------------------------------------------------------------------
alter table public.engagement_checklists enable row level security;

create policy "Engagement participants can read checklist"
  on public.engagement_checklists for select
  using (
    engagement_id in (
...
-- Writes go through service client (bypasses RLS)

-- ---------------------------------------------------------------------------
-- 4. Update apply_projection with checklist event handlers
-- ---------------------------------------------------------------------------
