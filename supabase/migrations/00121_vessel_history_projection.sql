-- Vessels V2 — Wave B: projection wiring for the new history events.
--
-- Wave A (migration 00120) added the `vessel_names` + `vessel_flag_states`
-- timeline tables and the per-vessel admin-enrichment columns. This
-- migration teaches `apply_projection` to:
--
--   1. Extend VESSEL.CREATED — also write the new `source` +
--      `submitted_by` columns on `vessels`, AND seed an open-ended
--      `vessel_names` row so Wave F's date-overlap query has data
--      from day one. (Wave A back-filled rows for the 19 existing
--      vessels; this keeps every NEW vessel populated automatically.)
--   2. Add VESSEL.RENAMED — closes the previously-open `vessel_names`
--      row by setting `effective_to = effective_from - 1 day`, inserts
--      a new open-ended row with the new name, updates the denormalised
--      `vessels.name` so existing readers don't need to know about the
--      timeline.
--   3. Add VESSEL.REFLAGGED — same pattern for `vessel_flag_states`.
--      Updates denormalised `vessels.flag_state_id`.
--   4. Add VESSEL.METADATA_UPDATED — admin-enrichment writes for
--      `gross_tonnage`, `beam_meters`, `year_built`, `builder` on
--      `vessels`. Each field uses `case when ... then ... else ... end`
--      so an explicit null in the payload is honoured (admin removing
--      a wrong year is distinct from omitting the field).
--
-- VESSEL.UPDATED is left untouched — existing call sites that edit a
-- vessel name continue to use it without history bookkeeping. Wave D
-- will migrate those callsites to emit VESSEL.RENAMED instead, after
-- which a future migration can tighten or remove the legacy handler.
--
-- The function body below is otherwise an exact copy of the latest
-- `apply_projection` definition (00116_multi_nationality_projection.sql)
-- with ONLY the VESSEL.CREATED handler modified and three new event
-- types appended after VESSEL.UPDATED.
...
begin
  case p_event_type
    ...
    when 'PERSON.DATA_SCRUBBED' then
      -- Side tables: documents, channels, placements, shore experiences, notes
      ...
      -- Full profile wipe — nulls every PII field, keeps display_name as the
      -- admin-list label. identity_type preserved so the audit trail retains
      -- whether the scrubbed account was a crew member or agent.
      ...
      set display_name = 'Deleted User',
          ...
          updated_at = now()
      where person_id = p_person_id;
      -- Null the onboarding gate so the JWT hook emits onboarded=false and
      -- middleware routes the user to /onboarding on next sign-in.
    ...
    when 'PROFILE.CREATED' then
      insert into public.profiles (
        ...
        -- nationality_id (legacy single column): prefer first element of
        -- nationality_ids if provided, fall back to legacy nationality_id
        coalesce(
          ((p_payload->'nationality_ids'->>0))::uuid,
          (p_payload->>'nationality_id')::uuid
        ),
        -- nationality_ids (new array column): prefer the array if provided,
        -- else wrap the legacy single field, else empty array
        coalesce(
    ...
    when 'PROFILE.UPDATED' then
      update public.profiles set
        ...
        -- nationality_ids (new array): explicit replace if payload has the
        -- array; else wrap the legacy single value if present; else keep
        -- existing.
        ...
        -- nationality_id (legacy single): prefer first element of new
        -- array, fall back to legacy field, else keep existing.
    ...
    when 'VESSEL.CREATED' then
      ...
      -- Seed the timeline so Wave F's overlap query always has data, even
      -- if the vessel never receives a RENAMED event.
    ...
    when 'VESSEL.RENAMED' then
      -- Close the previously-open name row (its effective range ends the
      -- day before the new name takes effect). The GREATEST clamp keeps
      -- same-day renames legal: if an admin emits RENAMED on the same
      -- day a vessel was created, `effective_from - 1 day` would
      -- underflow the row's existing `effective_from`, violating the
      -- interval CHECK. We clamp to a zero-length interval instead.
      -- Idempotent — if no open row exists, the UPDATE is a no-op.
      ...
      -- Insert the new row. Defaults to open-ended (effective_to=null)
      -- unless the payload explicitly sets effective_to (used by admins
      -- back-filling historical aliases inline during review).
      ...
      -- Update the denormalised "current name" if the new row is
      -- open-ended (i.e. it IS the current name now).
    ...
    when 'VESSEL.REFLAGGED' then
      -- GREATEST clamp: see VESSEL.RENAMED above.
    ...
    when 'VESSEL.METADATA_UPDATED' then
      -- Admin-enrichment writes. Each field uses `case when ? then ...`
      -- so explicit nulls in the payload are honoured (clears a value)
      -- but omitted fields stay as-is.
