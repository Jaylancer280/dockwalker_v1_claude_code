-- =============================================================================
-- Migration 00016: Remove message hiding functionality
-- =============================================================================
-- Message hiding has no place in a truth-centric app. All messages are
-- append-only and permanently visible to engagement participants.
--
-- Changes:
--   1. Drop hidden_by column from messages table
--   2. Update RLS policy to remove hidden_by filter
--   3. Remove MESSAGE.HIDDEN case from apply_projection
-- =============================================================================

-- 1. Drop old RLS policy that references hidden_by (must drop before column)
drop policy if exists "Engagement participants can read messages" on public.messages;

-- 2. Drop hidden_by column
alter table public.messages drop column if exists hidden_by;
-- 3. Recreate messages RLS policy without hidden_by filter
...
-- 4. Update apply_projection to remove MESSAGE.HIDDEN handler
