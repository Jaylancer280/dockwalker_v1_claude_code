-- =============================================================================
-- Migration 00130: NDA references — drop write-time gate, mask at display layer
--
-- Drops the NDA block from REFERENCE.REQUESTED. Original logic forbade
-- references on NDA vessels; the rule was internally incoherent — the
-- referee was the captain/HOD on that vessel, they obviously already knew
-- the IMO, and hiding it from them made verification impossible. NDA
-- protects vessel identity from outside parties; it has no purpose
-- between the requester and the referee, who are both insiders.
--
-- Protection moves to the display layer (`/api/messages/[engagementId]/context`):
--   • Referee always sees full snapshot.
--   • Employer-side caller in a reference-contact chat sees `'NDA Vessel'`
--     + null IMO unless they have a separate active engagement on that
--     vessel with the crew member (existing trust-boundary precedent
--     from get_vessel_public / 00083).
--
-- The candidate-profile API (`/api/profile/[personId]`) already strips
-- `snapshot_vessel_name`/`snapshot_vessel_imo` from the references[]
-- array, so no change is needed there. Self-views (`/api/references/mine`,
-- `/api/account/export`, the referee preview at `/ref/[token]`, and the
-- consent-prompts on `/messages`) are party-only surfaces and stay
-- unmasked.
--
-- Body is the 00129 apply_projection re-creation, line-for-line, with the
-- 3-line NDA block removed from the REFERENCE.REQUESTED handler. Handler
