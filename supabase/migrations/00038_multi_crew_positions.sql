-- =============================================================================
-- Migration 00038: Multi-Crew Positions
--
-- 1. Add positions_available and positions_filled columns to dayworks
-- 2. Add positions_available column to daywork_templates
-- 3. Add DAYWORK.POSITIONS_UPDATED event handler
-- 4. Rewrite DAYWORK.ACCEPTED to support multi-crew (conditional fill logic)
-- 5. Update DAYWORK.POSTED to write positions_available
-- 6. Update DAYWORK.RELISTED to NOT reset positions_filled
-- 7. Update ENGAGEMENT.CANCELLED_BY_CREW to decrement positions_filled
-- 8. Update ENGAGEMENT.CANCELLED_BY_EMPLOYER to decrement positions_filled
-- 9. Update DAYWORK.CANCELLED_BY_EMPLOYER to cascade cancel active engagements
-- =============================================================================

-- ---------------------------------------------------------------------------
-- 1. Add columns to dayworks
-- ---------------------------------------------------------------------------
alter table public.dayworks
  add column if not exists positions_available int not null default 1
    check (positions_available >= 1 and positions_available <= 20),
  add column if not exists positions_filled int not null default 0;

-- ---------------------------------------------------------------------------
-- 2. Add column to daywork_templates
-- ---------------------------------------------------------------------------
alter table public.daywork_templates
  add column if not exists positions_available int not null default 1;

-- ---------------------------------------------------------------------------
-- 3. Update apply_projection with multi-crew logic
-- ---------------------------------------------------------------------------
...
begin
  case p_event_type
    ...
    when 'DAYWORK.POSTED' then
      insert into public.dayworks (id, poster_person_id, role_context, vessel_id, role_id, location_port_id, start_date, end_date, working_days, required_certification_ids, experience_bracket_id, day_rate, currency, meals, notes, positions_available)
