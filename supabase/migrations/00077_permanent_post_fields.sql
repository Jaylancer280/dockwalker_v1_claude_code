-- =============================================================================
-- Migration 00077: Add missing fields to permanent_postings and templates
--
-- Adds contract_type, contract_details, description, meals, positions_available,
-- positions_filled to permanent_postings. Same columns (minus positions_filled)
-- to permanent_templates. Updates PERMANENT.POSTED handler in apply_projection.
-- =============================================================================

-- 1. Add columns to permanent_postings
alter table public.permanent_postings
  add column contract_type text check (contract_type in ('permanent', 'rotational', 'seasonal', 'crossing', 'delivery', 'temporary')),
  add column contract_details text,
  add column description text,
  add column meals text[] not null default '{}',
  add column positions_available integer not null default 1 check (positions_available between 1 and 20),
  add column positions_filled integer not null default 0;

-- 2. Add columns to permanent_templates (no positions_filled)
...
-- 3. Update apply_projection — only the PERMANENT.POSTED handler changes
