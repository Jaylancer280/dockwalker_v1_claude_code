-- =============================================================================
-- Migration 00136: apply_projection rewrite for engagement-phase + shortlist
-- conversations (B-011 commit 2)
--
-- Builds on 00135's `phase` column on `active_engagements`. Re-creates
-- `apply_projection` with the full 00132 body PLUS:
--
--   1. NEW PERMANENT.SHORTLIST_CHAT_OPENED handler. Idempotent insert of a
--      phase='shortlist' engagement row for the (posting × crew) pair.
--      Guards: posting must be 'active', application must be 'shortlisted'
--      (race protection — if SELECTED landed first the chat-open is rejected).
--
--   2. MODIFIED PERMANENT.SELECTED. Now does the major-commitment cascade:
--      - Promote existing shortlist engagement to phase='active' if one exists
--        (employer chatted before selecting), else INSERT a fresh phase='active'
--        engagement (employer selected without chatting).
--      - For OTHER shortlist-phase engagements on this posting: flip
--        phase='closed', outcome='role_filled'. Insert a system message into
--        each (warm "position has been filled" copy via MESSAGE.SENT path).
--        Their applications flip to status='not_selected'.
--      - Set posting status='in_negotiation' (existing).
--      - Revoke any pending permanent invitations (existing flow extended).
--
--   3. MODIFIED PERMANENT.PLACEMENT_CONFIRMED. Thinner now — the cascade
--      already happened at SELECTED time, so this just flips the posting
--      status (in_negotiation → filled) and the selected applicant
--      (selected → placement_confirmed). The legacy 'others → not_selected'
--      fan-out is retained as a defensive no-op (covers ledger replay
--      against pre-00136 events that didn't fan-out at SELECTED).
--
--   4. MODIFIED PERMANENT.SELECTION_REVERTED. Decisive change per the
--      product spec: the reverted candidate is now REJECTED (not just
--      not_selected) — definitive, no auto-resurrect path. Other
--      shortlistees stay closed (they were closed at SELECTED time).
--      Posting goes back to 'active' so the employer can attract new
--      applicants, but they cannot pick another from the prior shortlist.
--
--   5. MODIFIED PERMANENT.CANCELLED_BY_EMPLOYER. Now also closes any
--      shortlist-phase engagements on this posting. Existing active-phase
--      cascade preserved.
--
--   6. PHASE GUARDS on engagement-state-machine events. Each of these
--      filters now includes `phase = 'active'` so shortlist-phase rows
--      cannot accept work-started, postponement, completion, ratings, or
--      checklist events. Defence in depth — UI strips the buttons; the
--      projection rejects them too:
--        ENGAGEMENT.POSTPONEMENT_PROPOSED / ACCEPTED / REJECTED
--        ENGAGEMENT.WORK_STARTED / WORK_STARTED_CONFIRMED
--        ENGAGEMENT.COMPLETION_CONFIRMED / COMPLETION_DISPUTED
--        ENGAGEMENT.RATED_BY_CREW / EMPLOYER
--        ENGAGEMENT.CANCELLATION_RATED_BY_CREW / EMPLOYER
--        CHECKLIST.SET / ITEM_TOGGLED
--
-- Handler count: 85 → 86 (one new event type). $$ count = 2. Body ends
-- with `end case; end; $$;` per the lessons.md sanity check.
-- =============================================================================
...
declare
  ...
  -- B-011 shortlist-cascade locals
  v_application_id uuid;
  v_existing_engagement_id uuid;
  v_employer_person_id uuid;
  v_posting_start date;
  v_role_name text;
  v_system_message_body text;
  v_closing_engagement record;
begin
  case p_event_type
    ...
    when 'EXPERIENCE.UPDATED' then
      -- 00126/00129 extension: edit-lock on snapshot fields when active
      -- references exist on this experience (P0-A). vessel/role/start_date
      -- are unconditionally locked. end_date and is_current are locked
      -- EXCEPT for the one-time null→date / true→false transition that
      -- closes a currently-onboard experience.
      ...
      if v_active_ref_count > 0 then
        select vessel_id, role_id, start_date, end_date, is_current
          into v_existing_vessel, v_existing_role, v_existing_start, v_existing_end, v_existing_is_current
          from public.crew_experiences
          where id = p_aggregate_id::uuid and person_id = p_person_id;

        -- Vessel / role / start_date — always locked.
        ...
        then
          raise exception 'EXPERIENCE.UPDATED: cannot change vessel/role/start_date while % active reference(s) exist on this experience — revoke references first', v_active_ref_count;
        end if;

        -- end_date — only the null→date transition (closing a currently-onboard
        -- experience) is allowed. Any other change is locked.
        if p_payload ? 'end_date' and (p_payload->>'end_date')::date is distinct from v_existing_end then
          if not (
            v_existing_is_current
            and v_existing_end is null
            and (p_payload->>'end_date')::date is not null
          ) then
            raise exception 'EXPERIENCE.UPDATED: end_date is locked while % active reference(s) exist on this experience — revoke references first', v_active_ref_count;
          end if;
        end if;

        -- is_current — only the true→false transition is allowed (and only
        -- when the experience is currently-onboard with no end_date yet).
        if p_payload ? 'is_current' and (p_payload->>'is_current')::boolean is distinct from v_existing_is_current then
          if not (
            v_existing_is_current
            and v_existing_end is null
            and (p_payload->>'is_current')::boolean = false
          ) then
            raise exception 'EXPERIENCE.UPDATED: is_current is locked while % active reference(s) exist on this experience — revoke references first', v_active_ref_count;
          end if;
        end if;
      end if;
      ...
      -- 00129: when this update closed a currently-onboard experience
      -- (end_date null→date), propagate the new end_date to references whose
      -- snapshot was captured while the experience was open-ended. Without
      -- this, employers viewing the reference would see "Jan 2024 — Present"
      -- forever even after the user has long since left the vessel.
      if p_payload ? 'end_date' and (p_payload->>'end_date')::date is not null then
        update public.references
          set snapshot_end_date = (p_payload->>'end_date')::date
          where experience_id = p_aggregate_id::uuid
            and status in ('pending', 'accepted')
            and snapshot_end_date is null;
      end if;
    ...
    when 'ENGAGEMENT.POSTPONEMENT_PROPOSED' then
      -- B-011: phase guard — only active-phase engagements accept postponement.
      update public.active_engagements set postponement_status = 'proposed', proposed_start_date = (p_payload->>'proposed_start_date')::date, proposed_end_date = (p_payload->>'proposed_end_date')::date, proposed_working_days = (p_payload->>'proposed_working_days')::int where id = (p_payload->>'engagement_id')::uuid and status = 'active' and phase = 'active';

    when 'ENGAGEMENT.POSTPONEMENT_ACCEPTED' then
      -- B-011: phase guard — only active-phase engagements accept postponement.
      update public.active_engagements set start_date = (p_payload->>'new_start_date')::date, end_date = (p_payload->>'new_end_date')::date, postponement_status = 'accepted', proposed_start_date = null, proposed_end_date = null, proposed_working_days = null where id = (p_payload->>'engagement_id')::uuid and phase = 'active';
    ...
    when 'ENGAGEMENT.POSTPONEMENT_REJECTED' then
      -- B-011: phase guard — only active-phase engagements accept postponement.
      update public.active_engagements set status = 'cancelled', cancelled_by = 'postponement', postponement_status = 'rejected', cancellation_reason_category = 'postponement' where id = (p_payload->>'engagement_id')::uuid and phase = 'active';
    ...
    when 'ENGAGEMENT.WORK_STARTED' then
      -- B-011: phase guard — work_started only on active-phase engagements.
      update public.active_engagements set work_started_status = 'initiated_by_' || (p_payload->>'initiated_by') where id = (p_payload->>'engagement_id')::uuid and status = 'active' and phase = 'active' and work_started_status is null;

    when 'ENGAGEMENT.WORK_STARTED_CONFIRMED' then
      -- B-011: phase guard.
      update public.active_engagements set work_started_status = 'confirmed', work_started_at = now() where id = (p_payload->>'engagement_id')::uuid and status = 'active' and phase = 'active' and work_started_status in ('initiated_by_crew', 'initiated_by_employer');

    when 'ENGAGEMENT.COMPLETION_CONFIRMED' then
      -- B-011: phase guard. Completion only on active-phase engagements that
      -- already moved to status='completed' (the daywork-completed handler).
      update public.active_engagements set crew_completion_status = 'confirmed' where id = p_aggregate_id::uuid and crew_person_id = p_person_id and status = 'completed' and phase = 'active';

    when 'ENGAGEMENT.COMPLETION_DISPUTED' then
      -- B-011: phase guard.
      update public.active_engagements set crew_completion_status = 'disputed' where id = p_aggregate_id::uuid and crew_person_id = p_person_id and status = 'completed' and phase = 'active';

    when 'ENGAGEMENT.RATED_BY_CREW' then
      -- B-011: phase guard — ratings only on active-phase engagements.
      perform 1 from public.active_engagements where id = p_aggregate_id::uuid and phase = 'active';
      if not found then
        raise notice 'ENGAGEMENT.RATED_BY_CREW: engagement % is not phase=active, skipping', p_aggregate_id;
        return;
      end if;
    ...
    when 'ENGAGEMENT.RATED_BY_EMPLOYER' then
      ...
      perform 1 from public.active_engagements where id = p_aggregate_id::uuid and phase = 'active';
      if not found then
        raise notice 'ENGAGEMENT.RATED_BY_EMPLOYER: engagement % is not phase=active, skipping', p_aggregate_id;
        return;
      end if;
    ...
    when 'ENGAGEMENT.CANCELLATION_RATED_BY_CREW' then
      ...
      perform 1 from public.active_engagements where id = p_aggregate_id::uuid and phase = 'active';
      if not found then
        raise notice 'ENGAGEMENT.CANCELLATION_RATED_BY_CREW: engagement % is not phase=active, skipping', p_aggregate_id;
        return;
      end if;
    ...
    when 'ENGAGEMENT.CANCELLATION_RATED_BY_EMPLOYER' then
      ...
      perform 1 from public.active_engagements where id = p_aggregate_id::uuid and phase = 'active';
      if not found then
        raise notice 'ENGAGEMENT.CANCELLATION_RATED_BY_EMPLOYER: engagement % is not phase=active, skipping', p_aggregate_id;
        return;
      end if;
    ...
    when 'CHECKLIST.SET' then
      -- B-011: phase guard — checklists only on active-phase engagements.
      perform 1 from public.active_engagements where id = (p_payload->>'engagement_id')::uuid and phase = 'active';
      if not found then
        raise notice 'CHECKLIST.SET: engagement % is not phase=active, skipping', p_payload->>'engagement_id';
        return;
      end if;
    ...
    when 'CHECKLIST.ITEM_TOGGLED' then
      ...
      perform 1 from public.active_engagements where id = (p_payload->>'engagement_id')::uuid and phase = 'active';
      if not found then
        raise notice 'CHECKLIST.ITEM_TOGGLED: engagement % is not phase=active, skipping', p_payload->>'engagement_id';
        return;
      end if;
    ...
    when 'PERMANENT.APPLIED' then
      -- 00131 extension: invited_from_id is optional. When present (apply
      -- arriving via the PERMANENT.INVITED → /apply?from_invitation= flow),
      -- we persist the FK on applications AND flip the linked invitation
      -- row to 'applied' inside the same transaction. The `status='pending'`
      -- guard handles the concurrent-decline race — only one transition wins.
      insert into public.applications (id, crew_person_id, permanent_posting_id, status, message, invited_from_id) values ((p_payload->>'id')::uuid, (p_payload->>'crew_person_id')::uuid, (p_payload->>'permanent_posting_id')::uuid, 'applied', p_payload->>'message', case when p_payload ? 'invited_from_id' and (p_payload->>'invited_from_id') <> '' then (p_payload->>'invited_from_id')::uuid else null end);
      if p_payload ? 'invited_from_id' and (p_payload->>'invited_from_id') is not null and (p_payload->>'invited_from_id') <> '' then
        update public.permanent_invitations
          set status = 'applied', responded_at = now()
          where id = (p_payload->>'invited_from_id')::uuid
            and status = 'pending';
      end if;
    ...
    when 'PERMANENT.SHORTLISTED' then
      ...
      update public.applications set status = 'shortlisted', updated_at = now() where crew_person_id = (p_payload->>'crew_person_id')::uuid and permanent_posting_id = v_posting_id and status = 'applied';
    ...
    when 'PERMANENT.REJECTED' then
      ...
      -- B-011: if a shortlist-phase chat exists for this (posting × crew),
      -- close it. The chat thread persists for history; the message list
      -- shows it as read-only.
      update public.active_engagements
        set phase = 'closed', outcome = 'rejected'
        where permanent_posting_id = (p_payload->>'permanent_posting_id')::uuid
          and crew_person_id = (p_payload->>'crew_person_id')::uuid
          and phase = 'shortlist';

    when 'PERMANENT.SHORTLIST_CHAT_OPENED' then
      -- B-011: employer-initiated, opt-in shortlist chat. Idempotent on
      -- (posting × crew) — a re-tap returns silently without firing a
      -- second notification or creating a duplicate row. The route layer
      -- enforces this via the D-1 idempotency key
      -- `PERMANENT.SHORTLIST_CHAT_OPENED:${posting_id}:${crew_id}`.
      v_posting_id := (p_payload->>'permanent_posting_id')::uuid;
      v_application_id := (p_payload->>'application_id')::uuid;

      -- Race guard A: posting must still be active. If a sibling SELECTED
      -- already cascaded the shortlist, reject silently — the cascade
      -- already closed any chats and notified crew.
      perform 1 from public.permanent_postings where id = v_posting_id and status = 'active';
      if not found then
        raise notice 'PERMANENT.SHORTLIST_CHAT_OPENED: posting % not active, skipping', v_posting_id;
        return;
      end if;

      -- Race guard B: candidate must still be shortlisted.
      select status into v_app_status from public.applications where id = v_application_id;
      if v_app_status is distinct from 'shortlisted' then
        raise notice 'PERMANENT.SHORTLIST_CHAT_OPENED: application % is not shortlisted (status=%), skipping', v_application_id, coalesce(v_app_status, 'NOT_FOUND');
        return;
      end if;

      -- Idempotent insert. application_id is unique per (crew × posting)
      -- on `applications`; we use it as the upsert key. ON CONFLICT clause
      -- handles the rare retry that beats the D-1 key.
      select employer_person_id, start_date into v_employer_person_id, v_posting_start
        from public.permanent_postings where id = v_posting_id;

      insert into public.active_engagements (
        id, application_id, crew_person_id, employer_person_id,
        permanent_posting_id, start_date, end_date, status, phase
      )
      values (
        (p_payload->>'engagement_id')::uuid,
        v_application_id,
        (p_payload->>'crew_person_id')::uuid,
        v_employer_person_id,
        v_posting_id,
        v_posting_start,
        v_posting_start,
        'active',
    ...
    when 'PERMANENT.SELECTED' then
      ...
      -- Flip the selected application to 'selected'.
      update public.applications
        set status = 'selected', updated_at = now()
        where crew_person_id = (p_payload->>'crew_person_id')::uuid
          and permanent_posting_id = v_posting_id
          and status in ('applied', 'shortlisted');

      -- B-011: promote the existing shortlist-phase engagement (if employer
      -- chatted before selecting) to phase='active'. Otherwise INSERT a
      -- fresh phase='active' engagement (employer selected without chatting).
      update public.active_engagements ae
        set phase = 'active'
        from public.applications a
        where ae.application_id = a.id
          and a.crew_person_id = (p_payload->>'crew_person_id')::uuid
          and a.permanent_posting_id = v_posting_id
          and ae.phase = 'shortlist';
      ...
      select
        (p_payload->>'engagement_id')::uuid,
        a.id, a.crew_person_id, pp.employer_person_id, pp.id,
        pp.start_date, pp.start_date, 'active', 'active'
      from public.applications a
      join public.permanent_postings pp on pp.id = a.permanent_posting_id
      where a.crew_person_id = (p_payload->>'crew_person_id')::uuid
        and a.permanent_posting_id = v_posting_id
        and not exists (
          select 1 from public.active_engagements
          where application_id = a.id
        );

      -- B-011 cascade: close OTHER shortlist-phase engagements on this
      -- posting. Each closed thread also gets a system message inserted
      -- (warm "position has been filled" copy). Their applications flip
      -- to status='not_selected' so the rejection-notification dispatcher
      -- fires per crew.
      v_system_message_body := 'Unfortunately this position has been filled. Thank you for taking the time to apply and chat — your profile stays on DockWalker, and similar permanent roles post weekly. Best of luck with your search.';

      for v_closing_engagement in
        select ae.id as engagement_id, ae.crew_person_id
        from public.active_engagements ae
        where ae.permanent_posting_id = v_posting_id
          and ae.phase = 'shortlist'
          and ae.crew_person_id <> (p_payload->>'crew_person_id')::uuid
      loop
        -- Insert the warm closure system message into the closed thread
        -- using gen_random_uuid() as the message id (no event id needed —
        -- the cascade is part of SELECTED's transaction, not its own event).
        insert into public.messages (id, engagement_id, sender_person_id, content, is_system)
        values (gen_random_uuid(), v_closing_engagement.engagement_id, p_person_id, v_system_message_body, true);
      end loop;

      -- Bulk-close the shortlist-phase engagements + flip applications.
      update public.active_engagements
        set phase = 'closed', outcome = 'role_filled'
        where permanent_posting_id = v_posting_id
          and phase = 'shortlist'
          and crew_person_id <> (p_payload->>'crew_person_id')::uuid;
      ...
      -- Posting goes to in_negotiation for the negotiation period.
    ...
    when 'PERMANENT.PLACEMENT_CONFIRMED' then
      ...
      -- B-011: the SELECTED handler already cascaded other applications to
      -- 'not_selected', but keep this fan-out as a defensive no-op for
      -- ledger replay against pre-00136 events that didn't fan out at SELECTED.
    ...
    when 'PERMANENT.SELECTION_REVERTED' then
      ...
      -- B-011: decisive change. The reverted candidate is now REJECTED
      -- (not just not_selected) — definitive, no auto-resurrect path. Other
      -- shortlistees were already cascaded to not_selected at SELECTED time
      -- and stay closed. Posting goes back to 'active' so the employer can
      -- attract NEW applicants, but the prior shortlist is gone.
      update public.applications
        set status = 'rejected',
            rejection_reason = 'Selection reverted',
            updated_at = now()
        where permanent_posting_id = v_posting_id and status = 'selected';
    ...
    when 'PERMANENT.WITHDRAWN' then
      ...
      -- B-011: also close shortlist-phase chat (if any) for this withdrawing crew.
      update public.active_engagements
        set phase = 'closed', outcome = 'withdrew'
        where permanent_posting_id = v_posting_id
          and crew_person_id = (p_payload->>'crew_person_id')::uuid
          and phase = 'shortlist';
    ...
    when 'PERMANENT.CANCELLED_BY_EMPLOYER' then
      ...
      -- B-011: close any shortlist-phase chats on this cancelled posting.
      -- Each thread gets a system message announcing the cancellation.
      v_system_message_body := 'This posting has been cancelled by the employer. Thank you for your time — please continue exploring permanent roles on DockWalker.';
      for v_closing_engagement in
        select ae.id as engagement_id, ae.crew_person_id
        from public.active_engagements ae
        where ae.permanent_posting_id = v_posting_id and ae.phase = 'shortlist'
      loop
        insert into public.messages (id, engagement_id, sender_person_id, content, is_system)
        values (gen_random_uuid(), v_closing_engagement.engagement_id, p_person_id, v_system_message_body, true);
      end loop;
      update public.active_engagements
        set phase = 'closed', outcome = 'posting_cancelled'
        where permanent_posting_id = v_posting_id and phase = 'shortlist';
    ...
    when 'PERMANENT.INVITED' then
      -- 00132 audit P1-D1: state precondition guard — only INSERT when the
      -- posting is currently active. Matches the API-layer status check at
      -- /api/permanent/[id]/invite/route.ts:85-90. Without this guard, ledger
      -- replay against a re-seeded DB where the posting later reached
      -- in_negotiation/cancelled/filled would silently create invitations
      -- against stale postings.
      perform 1 from public.permanent_postings
        where id = (p_payload->>'permanent_posting_id')::uuid
          and status = 'active';
      if not found then
        raise notice 'PERMANENT.INVITED ignored: posting % not active', (p_payload->>'permanent_posting_id')::uuid;
        return;
      end if;

      insert into public.permanent_invitations (
        id, permanent_posting_id, crew_person_id, invited_by_person_id, status, message
      ) values (
        (p_payload->>'id')::uuid,
        (p_payload->>'permanent_posting_id')::uuid,
        (p_payload->>'crew_person_id')::uuid,
        p_person_id,
        'pending',
    ...
    when 'REFERENCE.REQUESTED' then
      ...
      -- 00130: NDA write-time gate dropped. NDA protects vessel identity
      -- from outside parties; it has no purpose between requester and
      -- referee (both are insiders). Display-layer mask handles employer
      -- visibility — see /api/messages/[engagementId]/context.
      if v_vessel_hidden is not null then
        raise exception 'REFERENCE.REQUESTED: vessel is hidden';
      end if;
      -- 00129: existence check only. The B-2 `is_current=true` block was
      -- dropped because first-vessel crew need to gather references from
      -- their current captain before they can secure a second job. Snapshot
      -- end_date is allowed to be null at request time and is auto-updated
      -- by EXPERIENCE.UPDATED when the experience is later closed.
    ...
    -- ── CV BUILDER EVENTS (00131) ─────────────────────────────────────────

    when 'CV.GENERATED' then
      -- Stamp generation timestamp; lazily backfill cv_handle if the route
      -- layer minted one and passed it through (older flows may not). If
      -- cv_handle is already set, the coalesce keeps it untouched. The
      -- cv_handle_updated_at stamp tracks first-mint or regenerate moments,
      -- which is what the stale-CV banner reads (spec §11). We DO NOT bump
      -- profiles.updated_at here — that field is the staleness signal:
      -- updated_at > cv_generated_at means "profile changed after CV was
      -- generated, show banner." Bumping it on generation would suppress
      -- the signal forever.
      update public.profiles
        set cv_generated_at = now(),
            cv_handle = coalesce(cv_handle, nullif(p_payload->>'handle', '')),
            cv_handle_updated_at = case
              when cv_handle is null and (p_payload->>'handle') is not null and (p_payload->>'handle') <> ''
                then now()
              else cv_handle_updated_at
            end
        where person_id = p_person_id;

    when 'CV.HANDLE_REGENERATED' then
      -- Crew Pro regenerates their CV link. The route mints the new handle
      -- and includes it in the payload so the projection just stamps. As
      -- with CV.GENERATED we do not bump profiles.updated_at — handle
      -- regeneration is not a profile data change.
      update public.profiles
        set cv_handle = p_payload->>'new_handle',
            cv_handle_updated_at = now()
        where person_id = p_person_id;
