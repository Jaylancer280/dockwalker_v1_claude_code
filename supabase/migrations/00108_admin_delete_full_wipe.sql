-- =============================================================================
-- Migration 00108: Admin Delete → full profile wipe + forced re-onboarding
--
-- Extends the `PERSON.DATA_SCRUBBED` projection handler so that an admin delete
-- (Stage 219 flow) nulls every PII field on the user's profile AND wipes their
-- crew/shore experience rows. A restored user lands on an empty profile with
-- `current_hat = null`, which the access token hook and middleware treat as
-- "not onboarded" — forcing the user through the onboarding flow again on
-- their next login.
--
-- Scope of the scrub after this migration:
--   profiles                    — display_name → 'Deleted User', every other
--                                 PII field nulled / emptied (22 fields)
--   persons                     — current_hat nulled (the onboarding gate)
--   crew_experiences            — all rows deleted (new)
--   shore_experiences           — all rows deleted (was already handled)
--   engagement_documents        — soft-deleted (unchanged)
--   notification_channels       — deleted (unchanged)
--   agent_placement_cities      — deleted (unchanged)
--   advisor_conversations       — deleted (unchanged)
--   docky_interactions          — person_id nulled, query/response scrubbed
--                                 (unchanged)
--   user_notes                  — content rewritten to '[content scrubbed]'
--                                 (unchanged — from 00106)
--
-- `custom_access_token_hook` is also updated so `onboarded` is `true` only
-- when the profile row exists AND `persons.current_hat is not null`. Existing
-- users (hat set) are unaffected.
--
-- This migration replaces `apply_projection` in its entirety. Handler count
-- remains 68, identical to 00106 except for the expanded PERSON.DATA_SCRUBBED
-- body. All other handlers are copied verbatim from 00106.
-- =============================================================================

-- 1. Replace apply_projection — PERSON.DATA_SCRUBBED expanded to full wipe.
...
-- 2. Replace custom_access_token_hook — onboarded is true only when both the
--    profile row exists AND persons.current_hat is set. All other claims
--    (person_id, current_hat, identity_type, deactivated, blocked, is_admin)
--    remain identical to 00105.
...
begin
  ...
  if person_record.id is not null then
    ...
    if exists (select 1 from public.profiles where person_id = person_record.id)
       and person_record.current_hat is not null then
