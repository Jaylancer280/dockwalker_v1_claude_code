-- =============================================================================
-- Migration 00029: Experience enhancements
--
-- 1. Vessels: rename vessel_type -> vessel_operation, add vessel_type (motor/sail)
-- 2. Crew experiences: rename charter_or_private -> vessel_operation,
--    rename rotation_type -> contract_type with new values,
--    rename rotation_details -> contract_details
-- 3. Update get_vessel_public to return vessel_operation + vessel_type
-- 4. Update apply_projection for all renamed columns
-- =============================================================================
...
-- Rename existing vessel_type column to vessel_operation
alter table public.vessels rename column vessel_type to vessel_operation;

-- Drop old CHECK and add new one (same values, new column name)
alter table public.vessels drop constraint vessels_vessel_type_check;
alter table public.vessels add constraint vessels_vessel_operation_check
  check (vessel_operation in ('private', 'charter'));

-- Add new vessel_type column for motor/sail classification
alter table public.vessels add column vessel_type text
  check (vessel_type in ('motor', 'sail'));

-- Default existing vessels to motor (most superyachts are motor)
update public.vessels set vessel_type = 'motor' where vessel_type is null;

-- Make vessel_type NOT NULL after backfill
alter table public.vessels alter column vessel_type set not null;

-- -----------------------------------------------------------------------------
-- 2. Crew experiences: rename charter_or_private -> vessel_operation
-- -----------------------------------------------------------------------------
alter table public.crew_experiences rename column charter_or_private to vessel_operation;

-- Drop old CHECK and add new one
alter table public.crew_experiences drop constraint crew_experiences_charter_or_private_check;
alter table public.crew_experiences add constraint crew_experiences_vessel_operation_check
  check (vessel_operation in ('charter', 'private'));

-- -----------------------------------------------------------------------------
-- 3. Crew experiences: rename rotation_type -> contract_type with new values,
--    rename rotation_details -> contract_details
-- -----------------------------------------------------------------------------
alter table public.crew_experiences rename column rotation_type to contract_type;
alter table public.crew_experiences rename column rotation_details to contract_details;

-- Drop old CHECK constraints
alter table public.crew_experiences drop constraint crew_experiences_rotation_type_check;
-- rotation_details only had a length check, renaming preserves it

-- Add new contract_type CHECK with updated values
alter table public.crew_experiences add constraint crew_experiences_contract_type_check
  check (contract_type in ('permanent', 'rotational', 'seasonal', 'crossing', 'delivery', 'temporary'));

-- Migrate existing rotation_type values to new contract_type values
-- Rotation patterns (2:2, 3:1, 3:3, 5:1) -> 'rotational'
-- permanent -> permanent
-- seasonal -> seasonal
-- mlc_standard -> rotational (MLC defines rotation schedules)
-- other -> temporary (closest general-purpose match)
-- NOTE: Must remove CHECK first, do update, then re-add CHECK
alter table public.crew_experiences drop constraint crew_experiences_contract_type_check;

update public.crew_experiences set contract_type = 'rotational'
  where contract_type in ('2:2', '3:1', '3:3', '5:1', 'mlc_standard');
update public.crew_experiences set contract_type = 'temporary'
  where contract_type = 'other';
-- permanent and seasonal keep their values

alter table public.crew_experiences add constraint crew_experiences_contract_type_check
  check (contract_type in ('permanent', 'rotational', 'seasonal', 'crossing', 'delivery', 'temporary'));

-- -----------------------------------------------------------------------------
-- 4. Update get_vessel_public to return vessel_operation + vessel_type
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 5. Update apply_projection for all renamed columns
-- -----------------------------------------------------------------------------
...
begin
  case p_event_type
    ...
    when 'VESSEL.CREATED' then
      insert into public.vessels (id, owner_person_id, imo_number, name, vessel_type, vessel_operation, size_band_id, loa_meters, nda_flag)
      values (
        (p_payload->>'id')::uuid,
        p_person_id,
        ...
        coalesce(p_payload->>'vessel_operation', p_payload->>'vessel_type', 'charter'),
