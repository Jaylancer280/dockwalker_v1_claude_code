-- =============================================================================
-- Migration 00018: Structured employer cancellation mechanic
-- =============================================================================
-- Transforms employer cancellation from a bare confirm dialog into a structured,
-- reason-driven flow with crew notification, postponement proposals, relist logic,
-- and cancellation-context ratings.
-- =============================================================================

-- ---------------------------------------------------------------------------
-- 1. Alter active_engagements: cancellation + postponement columns
-- ---------------------------------------------------------------------------
alter table public.active_engagements
  add column cancellation_reason_category text
  ...
  add column relist_requested boolean,
  add column relist_reason_category text
    check (relist_reason_category in ('wrong_crew', 'requirements_changed', 'different_skills', 'relist_other')),
  add column relist_reason_text text,
  add column postponement_status text
    check (postponement_status in ('proposed', 'accepted', 'rejected')),
  add column proposed_start_date date,
  add column proposed_end_date date,
  add column proposed_working_days int;

-- ---------------------------------------------------------------------------
-- 2. Alter messages: system message flag
-- ---------------------------------------------------------------------------
alter table public.messages
  add column is_system boolean not null default false;

-- ---------------------------------------------------------------------------
-- 3. Alter engagement_ratings: rating context + notice_given
-- ---------------------------------------------------------------------------
alter table public.engagement_ratings
  add column rating_context text not null default 'completed'
    check (rating_context in ('completed', 'cancelled')),
  add column notice_given text
    check (notice_given in ('yes', 'no', 'partial'));

-- ---------------------------------------------------------------------------
-- 4. New RPC: check_no_overlap_excluding
-- ---------------------------------------------------------------------------
create or replace function public.check_no_overlap_excluding(
  p_crew_person_id uuid,
  p_start_date date,
  p_end_date date,
  p_exclude_engagement_id uuid
)
returns boolean
language sql
security definer
...
as $$
  select not exists (
    ...
    where ae.crew_person_id = p_crew_person_id
    and ae.status = 'active'
    and ae.id != p_exclude_engagement_id
    and ae.start_date <= p_end_date
    and ae.end_date >= p_start_date
  );
$$;

-- ---------------------------------------------------------------------------
-- 5. Update apply_projection with new event handlers
-- ---------------------------------------------------------------------------
  ...
  case p_event_type
    ...
    when 'ENGAGEMENT.CANCELLED_BY_EMPLOYER' then
      ...
      set status = 'cancelled',
          ...
          relist_requested = coalesce((p_payload->>'relist_requested')::boolean, false),
          relist_reason_category = p_payload->>'relist_reason_category',
          relist_reason_text = p_payload->>'relist_reason_text'
      where crew_person_id = (split_part(p_aggregate_id, ':', 1))::uuid
      and daywork_id = (split_part(p_aggregate_id, ':', 2))::uuid;

    when 'ENGAGEMENT.POSTPONEMENT_PROPOSED' then
      update public.active_engagements
      set postponement_status = 'proposed',
          proposed_start_date = (p_payload->>'proposed_start_date')::date,
          proposed_end_date = (p_payload->>'proposed_end_date')::date,
          proposed_working_days = (p_payload->>'proposed_working_days')::int
    ...
    when 'ENGAGEMENT.POSTPONEMENT_ACCEPTED' then
      ...
      update public.active_engagements
      set start_date = (p_payload->>'new_start_date')::date,
          end_date = (p_payload->>'new_end_date')::date,
          postponement_status = 'accepted',
          proposed_start_date = null,
          proposed_end_date = null,
          proposed_working_days = null
      where id = (p_payload->>'engagement_id')::uuid;

      -- Update daywork dates
      update public.dayworks
      set start_date = (p_payload->>'new_start_date')::date,
          end_date = (p_payload->>'new_end_date')::date,
          working_days = (p_payload->>'new_working_days')::int
      where id = (p_payload->>'daywork_id')::uuid;
    ...
    when 'ENGAGEMENT.POSTPONEMENT_REJECTED' then
      ...
      set status = 'cancelled',
          ...
          cancellation_reason_category = 'postponement'
      where id = (p_payload->>'engagement_id')::uuid;
    ...
    when 'ENGAGEMENT.CANCELLATION_RATED_BY_CREW' then
      insert into public.engagement_ratings (
        engagement_id, rater_person_id, rater_role, rating_context,
        notice_given, communication_accuracy, overall_match
      )
      values (
        p_aggregate_id::uuid,
        p_person_id,
