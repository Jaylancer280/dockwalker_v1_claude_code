-- =============================================================================
-- Migration 00081: Docky interaction logging + GDPR advisor scrub
--
-- 1. Creates docky_interactions table for analytics/quality monitoring.
--    Service-role only (no user-facing RLS policies).
--    90-day retention policy — cleanup cron deferred to when volume warrants it.
--
-- 2. Fixes pre-existing GDPR gap: DATA_SCRUBBED now deletes advisor_conversations
--    (messages cascade) and scrubs docky_interactions.
-- =============================================================================

-- 1. Create docky_interactions table
...
alter table public.docky_interactions enable row level security;

comment on table public.docky_interactions is
  'Docky advisor interaction log. Service-role insert only. 90-day retention (cleanup cron deferred).';

-- Index for person lookups (GDPR scrub, analytics)
create index idx_docky_interactions_person on public.docky_interactions (person_id);

-- 2. Update apply_projection — add DATA_SCRUBBED handler with advisor + interactions scrub
