-- =============================================================================
-- Migration 00131: CV Builder v1 — schema + projection extensions
--
-- Phase 1 of the CV Builder feature. Spec: tasks/cv-builder-spec.md (v2.1).
-- Stage 1 of the staged rollout (Stage 2 ships PDF render + lockdown).
--
-- This migration is schema-only + projection extensions. No route handlers,
-- no UI. The CV section in /settings/cv, the QR-landing route, and the
-- hire-from-QR wizards are wired in later phases. The Generate button stays
-- greyed out and the /api/cv/generate route returns 503 until Stage 2.
--
-- What ships here:
--   • Profile-level CV settings: cv_handle (8-char unique), cv_handle_updated_at,
--     cv_include_sea_time toggle, cv_generated_at stamp.
--   • Per-experience NDA disclosure toggle: cv_show_full_vessel (default true).
--   • Per-reference inclusion-on-CV toggle: include_on_cv (default false).
--   • Backlink so a permanent application can carry attribution to the
--     invitation that produced it: applications.invited_from_id.
--   • New permanent_invitations table mirroring daywork_invitations (RLS,
--     indexes, lifecycle states pending/applied/declined/revoked/expired).
--   • events.aggregate_type allow-list extended with 'permanent_invitation'.
--   • apply_projection extended with 3 new handlers (PERMANENT.INVITED,
--     CV.GENERATED, CV.HANDLE_REGENERATED) and PERMANENT.APPLIED extended
--     with the optional invited_from_id branch.
--
-- daywork_invitations.UNIQUE(daywork_id, crew_person_id) already exists
-- (constraint daywork_invitations_unique_crew_daywork from migration 00030).
-- We do NOT recreate it here — the spec called for idempotency at the
-- application layer; the database already provides it.
--
-- Lessons-mandated apply_projection replacement protocol: $$ count = 2,
-- ends with `end case; end; $$;`. Body is line-for-line copy of 00130 with
-- 4 changes:
--   1. PERMANENT.APPLIED handler extended (optional invited_from_id branch).
--   2. PERMANENT.INVITED inserted after PERMANENT.ENGAGEMENT_CLOSED.
--   3. CV.GENERATED inserted after the reference handlers.
--   4. CV.HANDLE_REGENERATED inserted after CV.GENERATED.
-- Handler count: 82 → 85.
-- =============================================================================

-- ---------------------------------------------------------------------------
-- 1. profiles — CV-specific columns
-- ---------------------------------------------------------------------------
...
-- Partial index for fast lookups against /api/cv/[handle] — most rows have null
create index idx_profiles_cv_handle
  on public.profiles(cv_handle)
  where cv_handle is not null;

-- ---------------------------------------------------------------------------
-- 2. crew_experiences — per-experience NDA disclosure toggle
-- ---------------------------------------------------------------------------
-- Default true (industry norm is to disclose vessel name on a CV); the toggle
-- only surfaces in the UI when the bound vessel has nda_flag = true.
alter table public.crew_experiences
  add column cv_show_full_vessel boolean not null default true;

-- ---------------------------------------------------------------------------
-- 3. references — per-reference inclusion-on-CV toggle
-- ---------------------------------------------------------------------------
-- Default false (privacy default — explicit opt-in even though the referee
-- has already consented to display the reference on the platform).
alter table public.references
  add column include_on_cv boolean not null default false;

-- ---------------------------------------------------------------------------
-- 4. permanent_invitations — new table mirroring daywork_invitations
-- ---------------------------------------------------------------------------
...
create index idx_permanent_invitations_crew
  on public.permanent_invitations(crew_person_id, status);
create index idx_permanent_invitations_posting
  on public.permanent_invitations(permanent_posting_id);
-- Partial index that powers the daily expiry cron (Phase 6) — pending-only.
...
create policy "permanent_invitations_select_party_or_poster"
  ...
  using (
    ...
    or exists (
      ...
      where pp.id = permanent_posting_id
        and pp.employer_person_id = auth.uid()
    )
  );

-- Writes flow through apply_projection as service role; no INSERT/UPDATE/DELETE
-- policies for authenticated users (matches daywork_invitations pattern).

-- ---------------------------------------------------------------------------
-- 5. applications — invited_from_id backlink
-- ---------------------------------------------------------------------------
-- Created AFTER permanent_invitations so the FK target exists.
alter table public.applications
  add column invited_from_id uuid
    references public.permanent_invitations(id) on delete set null;

create index idx_applications_invited_from
  on public.applications(invited_from_id)
  where invited_from_id is not null;

-- ---------------------------------------------------------------------------
-- 6. events.aggregate_type — extend allow-list with 'permanent_invitation'
-- ---------------------------------------------------------------------------
...
alter table public.events add constraint events_aggregate_type_check
  check (aggregate_type in (
    ...
    'reference', 'reference_contact', 'permanent_invitation'
  ));

-- ---------------------------------------------------------------------------
-- 7. apply_projection — full re-creation with CV Builder extensions
-- ---------------------------------------------------------------------------
-- Body is the 00130 apply_projection re-creation, line-for-line, with:
--   • PERMANENT.APPLIED handler extended (optional invited_from_id branch
--     persists the FK on applications and flips the linked invitation row
--     to 'applied' inside the same transaction; no-op when payload field
--     is null, so all existing apply flows continue to work unchanged).
--   • PERMANENT.INVITED handler appended after PERMANENT.ENGAGEMENT_CLOSED.
--   • CV.GENERATED + CV.HANDLE_REGENERATED appended after the reference
--     handlers.
-- Handler count: 82 → 85.
...
begin
  case p_event_type
    ...
    when 'PERMANENT.INVITED' then
      -- 00131: captain/agent invites a specific crew to apply to a permanent
      -- posting. The aggregate is the new permanent_invitation row. The route
      -- layer guarantees there is no duplicate (UNIQUE(posting, crew) constraint
      -- below) — re-invitation surfaces a 409 from the API, never reaches here.
