-- Migration 00024: Availability city + not-available state
-- Adds city_id and not_available to availability_windows for town-level location
-- and explicit "not available" declaration on crew availability.
-- Updates apply_projection to write city_id and not_available on AVAILABILITY.SET.

-- ---------------------------------------------------------------------------
-- 1. Add city_id and not_available columns to availability_windows
-- ---------------------------------------------------------------------------
alter table public.availability_windows
  add column city_id uuid references public.cities(id);

alter table public.availability_windows
  add column not_available boolean not null default false;

create index idx_availability_city on public.availability_windows (city_id);

-- ---------------------------------------------------------------------------
-- 2. Update apply_projection to include city_id in AVAILABILITY.SET handler
-- ---------------------------------------------------------------------------
...
begin
  case p_event_type
    ...
    -- Work started handshake
    when 'ENGAGEMENT.WORK_STARTED' then
      update public.active_engagements
      set work_started_status = 'initiated_by_' || (p_payload->>'initiated_by')
      where id = (p_payload->>'engagement_id')::uuid
      and status = 'active'
      and work_started_status is null;

    when 'ENGAGEMENT.WORK_STARTED_CONFIRMED' then
      update public.active_engagements
      set work_started_status = 'confirmed',
          work_started_at = now()
      where id = (p_payload->>'engagement_id')::uuid
      and status = 'active'
      and work_started_status in ('initiated_by_crew', 'initiated_by_employer');
    ...
    when 'CHECKLIST.SET' then
      insert into public.engagement_checklists (engagement_id, items, acknowledged_item_ids, updated_at)
      values (
    ...
    when 'CHECKLIST.ITEM_TOGGLED' then
      if (p_payload->>'checked')::boolean then
        update public.engagement_checklists
        set acknowledged_item_ids = array_append(
              array_remove(acknowledged_item_ids, p_payload->>'item_id'),
              p_payload->>'item_id'
            ),
            updated_at = now()
        where engagement_id = (p_payload->>'engagement_id')::uuid;
      else
        update public.engagement_checklists
        set acknowledged_item_ids = array_remove(acknowledged_item_ids, p_payload->>'item_id'),
            updated_at = now()
        where engagement_id = (p_payload->>'engagement_id')::uuid;
      end if;
    ...
    when 'AVAILABILITY.SET' then
      if coalesce((p_payload->>'not_available')::boolean, false) then
        -- Explicit "not available" declaration: expire all existing windows
        -- and insert a single marker row for today
        update public.availability_windows
        set expires_at = now()
        where person_id = p_person_id
        and expires_at > now();

        insert into public.availability_windows (person_id, date, expires_at, city_id, not_available)
        values (
          p_person_id,
          current_date,
        ...
        -- Normal availability: clear any not_available marker first
        update public.availability_windows
        set expires_at = now()
        where person_id = p_person_id
