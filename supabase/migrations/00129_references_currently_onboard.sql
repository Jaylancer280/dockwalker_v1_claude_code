-- =============================================================================
-- Migration 00129: References on currently-onboard experiences
--
-- 00126's B-2 gate forbade references on `is_current=true` experiences. That
-- locks out crew on their first vessel — they cannot gather a single
-- reference until they leave, even though the captain they're working under
-- right now is the most valuable referee they will ever have.
--
-- This migration:
--   1. Drops the B-2 `is_current=true` block from REFERENCE.REQUESTED.
--      The handler still raises if the experience does not exist.
--   2. Loosens the EXPERIENCE.UPDATED edit-lock so a currently-onboard
--      experience with active references can be transitioned to completed:
--        • `end_date` may transition null → date (one-time only).
--        • `is_current` may transition true → false (one-time only).
--        • `vessel_id`, `role_id`, `start_date` remain unconditionally locked.
--      Once the experience is completed (end_date is a real date), all four
--      fields are locked exactly as 00126 enforced.
--   3. Auto-propagates the new end_date to the references' snapshot when
--      that null→date transition happens, so referee-verified records show
--      the correct closed period instead of "Jan 2024 — Present" forever.
--
-- The route layer (`/api/references` POST and `/api/experiences/[id]` PATCH)
-- is updated in lockstep; this migration is the projection-side defence in
-- depth.
--
-- Body is the 00128 apply_projection re-creation, line-for-line, with two
-- handlers modified (REFERENCE.REQUESTED, EXPERIENCE.UPDATED). Handler
