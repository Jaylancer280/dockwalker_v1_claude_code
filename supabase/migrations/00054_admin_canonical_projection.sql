-- =============================================================================
-- Migration 00054: Admin canonical projection handlers
--
-- Adds two audit-only no-op handlers for ADMIN.CANONICAL_ADDED and
-- ADMIN.CANONICAL_UPDATED to apply_projection. These events are emitted by
-- the admin canonical CRUD routes and recorded in the event ledger for audit
-- purposes. The projection handlers simply log a NOTICE confirming the event
-- was processed, preventing them from falling through to the "Unknown event
-- type" else branch.
--
-- Only the admin actions section is changed. All other handlers are identical
-- to migration 00053.
-- =============================================================================
...
begin
  case p_event_type
    ...
    when 'DAYWORK.ACCEPTED' then
      -- Race guard: check positions not already full before processing accept
      ...
      -- 1. Accept the application (only if in valid pre-acceptance state)
    ...
    when 'ADMIN.ENGAGEMENT_COMPLETED' then
      -- Same logic as DAYWORK.COMPLETED but reads daywork_id from engagement
    ...
    when 'ADMIN.CANONICAL_ADDED' then
      -- Audit-only: canonical data written directly by admin route
    ...
    when 'ADMIN.CANONICAL_UPDATED' then
      -- Audit-only: canonical data updated directly by admin route
