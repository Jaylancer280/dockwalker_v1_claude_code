-- =============================================================================
-- Migration 00059: Permanent jobs schema + types + events
--
-- Creates permanent_postings and permanent_templates tables, extends profiles
-- with permanent availability columns, extends applications and
-- active_engagements with permanent_posting_id + XOR constraints, updates
-- events_aggregate_type_check, adds RLS policies, and adds PERMANENT.*
-- handlers to apply_projection.
-- =============================================================================

-- =============================================================================
-- 1a. Create permanent_postings table
-- =============================================================================
create table public.permanent_postings (
...
create index idx_permanent_postings_employer on public.permanent_postings(employer_person_id);
create index idx_permanent_postings_status on public.permanent_postings(status);
create index idx_permanent_postings_port on public.permanent_postings(port_id);

-- =============================================================================
-- 1b. Create permanent_templates table
-- =============================================================================
create table public.permanent_templates (
  id uuid primary key default gen_random_uuid(),
  employer_person_id uuid not null references public.persons(id),
...
-- =============================================================================
alter table public.profiles add column permanent_availability text
  check (permanent_availability in ('immediate', 'after_notice', 'not_looking'));
alter table public.profiles add column notice_period_days integer;
alter table public.profiles add column currently_employed boolean default false;

-- =============================================================================
-- 1d. Extend applications table
-- =============================================================================
-- Make daywork_id nullable (was NOT NULL) for XOR with permanent_posting_id
alter table public.applications alter column daywork_id drop not null;

-- Drop existing unique constraint on (crew_person_id, daywork_id)
alter table public.applications drop constraint applications_crew_person_id_daywork_id_key;

-- Add partial unique indexes for each posting type
create unique index applications_crew_daywork_unique
  on public.applications (crew_person_id, daywork_id)
  where daywork_id is not null;

alter table public.applications add column permanent_posting_id uuid references public.permanent_postings(id);
alter table public.applications add column rejection_reason text;

create unique index applications_crew_permanent_unique
  on public.applications (crew_person_id, permanent_posting_id)
  where permanent_posting_id is not null;

-- Drop and recreate status check to add 'selected' and 'not_selected'
...
-- XOR: exactly one of daywork_id or permanent_posting_id must be set
alter table public.applications add constraint applications_posting_xor
  check ((daywork_id is not null) != (permanent_posting_id is not null));

create index idx_applications_permanent on public.applications(permanent_posting_id);

-- =============================================================================
-- 1e. Extend active_engagements table
-- =============================================================================
-- Make daywork_id nullable for XOR with permanent_posting_id
alter table public.active_engagements alter column daywork_id drop not null;

alter table public.active_engagements add column permanent_posting_id uuid references public.permanent_postings(id);
alter table public.active_engagements add column outcome text
  check (outcome in ('successful_placement', 'not_successful', 'withdrew'));

-- Drop and recreate status check to add 'closed'
alter table public.active_engagements drop constraint active_engagements_status_check;
alter table public.active_engagements add constraint active_engagements_status_check
  check (status in ('active', 'completed', 'cancelled', 'closed'));

-- XOR: exactly one of daywork_id or permanent_posting_id must be set
alter table public.active_engagements add constraint engagements_posting_xor
  check ((daywork_id is not null) != (permanent_posting_id is not null));

create index idx_engagements_permanent on public.active_engagements(permanent_posting_id);
...
-- =============================================================================
-- 1g. RLS on permanent_postings
-- =============================================================================
alter table public.permanent_postings enable row level security;
...
-- INSERT/UPDATE via service role only (apply_projection runs as security definer)

-- =============================================================================
-- 1h. RLS on permanent_templates
-- =============================================================================
alter table public.permanent_templates enable row level security;

create policy "Owner can read own permanent templates"
  on public.permanent_templates for select
  to authenticated
  using (employer_person_id = auth.uid());

create policy "Owner can insert own permanent templates"
  on public.permanent_templates for insert
  to authenticated
  with check (employer_person_id = auth.uid());

create policy "Owner can update own permanent templates"
  on public.permanent_templates for update
  to authenticated
  using (employer_person_id = auth.uid())
  with check (employer_person_id = auth.uid());

create policy "Owner can delete own permanent templates"
  on public.permanent_templates for delete
  to authenticated
  using (employer_person_id = auth.uid());

-- =============================================================================
-- 1i. RLS extension on applications for permanent postings
-- =============================================================================
create policy "Crew can read own permanent applications"
  on public.applications for select
  to authenticated
  using (crew_person_id = auth.uid() and permanent_posting_id is not null);

create policy "Employer can read applications on own permanent postings"
  on public.applications for select
  to authenticated
  using (permanent_posting_id in (
    select id from public.permanent_postings where employer_person_id = auth.uid()
  ));

-- =============================================================================
-- 1j. Update apply_projection with PERMANENT.* handlers
-- =============================================================================
-- CRITICAL: Full function replacement. All existing handlers preserved
-- character-identical from migration 00057. New PERMANENT.* handlers added
-- before the ELSE clause. PROFILE.UPDATED extended with permanent fields.
...
begin
  case p_event_type
    ...
    -- =========================================================================
    -- PERMANENT.* handlers — zero cross-contamination with daywork handlers
    -- =========================================================================

    when 'PERMANENT.POSTED' then
      insert into public.permanent_postings (
        id, employer_person_id, vessel_id, role_id, port_id, start_date,
        salary_min, salary_max, salary_currency, salary_period, live_aboard,
        required_certification_ids, experience_bracket_id, shortlist_cap, notes
      ) values (
        (p_payload->>'id')::uuid,
        p_person_id,
        (p_payload->>'vessel_id')::uuid,
        (p_payload->>'role_id')::uuid,
        (p_payload->>'port_id')::uuid,
        (p_payload->>'start_date')::date,
        (p_payload->>'salary_min')::numeric,
        (p_payload->>'salary_max')::numeric,
        p_payload->>'salary_currency',
        p_payload->>'salary_period',
        (p_payload->>'live_aboard')::boolean,
        coalesce((select array_agg(x::uuid) from jsonb_array_elements_text(p_payload->'required_certification_ids') x), '{}'),
        (p_payload->>'experience_bracket_id')::uuid,
        coalesce((p_payload->>'shortlist_cap')::int, 5),
        p_payload->>'notes'
      );

    when 'PERMANENT.APPLIED' then
      insert into public.applications (id, crew_person_id, permanent_posting_id, status, message)
      values (
        (p_payload->>'id')::uuid,
    ...
    when 'PERMANENT.SHORTLISTED' then
      ...
      -- Guard: check shortlist cap
      select count(*) into v_shortlist_count
        from public.applications
        where permanent_posting_id = v_posting_id
        and status in ('shortlisted', 'selected');
      select shortlist_cap into v_available
        from public.permanent_postings
        where id = v_posting_id;
      if v_shortlist_count >= v_available then
        raise notice 'Shortlist cap reached for permanent posting %, skipping', v_posting_id;
        return;
      end if;
    ...
    when 'PERMANENT.REJECTED' then
      update public.applications set status = 'rejected', updated_at = now()
        where crew_person_id = (p_payload->>'crew_person_id')::uuid
        and permanent_posting_id = (p_payload->>'permanent_posting_id')::uuid
        and status in ('applied', 'shortlisted');

    when 'PERMANENT.SELECTED' then
      v_posting_id := (p_payload->>'permanent_posting_id')::uuid;
      -- Guard: posting must be active, no other selected application
      perform 1 from public.permanent_postings where id = v_posting_id and status = 'active';
      if not found then
        raise notice 'Permanent posting % not active, skipping select', v_posting_id;
        return;
      end if;
      perform 1 from public.applications where permanent_posting_id = v_posting_id and status = 'selected';
      if found then
        raise notice 'Permanent posting % already has selected applicant, skipping', v_posting_id;
        return;
      end if;
      -- Update application to selected
      update public.applications set status = 'selected', updated_at = now()
        where crew_person_id = (p_payload->>'crew_person_id')::uuid
        and permanent_posting_id = v_posting_id
        and status in ('applied', 'shortlisted');
      -- Create engagement
      insert into public.active_engagements (id, application_id, crew_person_id, employer_person_id, permanent_posting_id, start_date, end_date, status)
        select (p_payload->>'engagement_id')::uuid, a.id, a.crew_person_id, pp.employer_person_id, pp.id, pp.start_date, pp.start_date, 'active'
      ...
      -- Move posting to in_negotiation
      update public.permanent_postings set status = 'in_negotiation', updated_at = now()
        where id = v_posting_id;

    when 'PERMANENT.PLACEMENT_CONFIRMED' then
      v_posting_id := (p_payload->>'permanent_posting_id')::uuid;
      -- Guard: posting must be in_negotiation
      perform 1 from public.permanent_postings where id = v_posting_id and status = 'in_negotiation';
      if not found then
        raise notice 'Permanent posting % not in_negotiation, skipping placement confirm', v_posting_id;
        return;
      end if;
      -- Fill the posting
      update public.permanent_postings set status = 'filled', updated_at = now()
        where id = v_posting_id;
      -- Not-select all remaining shortlisted/applied
      update public.applications set status = 'not_selected', rejection_reason = 'Position has been filled', updated_at = now()
        where permanent_posting_id = v_posting_id
        and status in ('applied', 'shortlisted');
    ...
    when 'PERMANENT.SELECTION_REVERTED' then
      ...
      perform 1 from public.permanent_postings where id = v_posting_id and status = 'in_negotiation';
      if not found then
        raise notice 'Permanent posting % not in_negotiation, skipping selection revert', v_posting_id;
        return;
      end if;
      -- Close engagement
      update public.active_engagements set status = 'closed', outcome = 'not_successful'
        where id = (p_payload->>'engagement_id')::uuid;
      -- Not-select the selected application
      update public.applications set status = 'not_selected', updated_at = now()
        where permanent_posting_id = v_posting_id and status = 'selected';
      -- Revert posting to active
      update public.permanent_postings set status = 'active', updated_at = now()
        where id = v_posting_id;

    when 'PERMANENT.WITHDRAWN' then
      v_posting_id := (p_payload->>'permanent_posting_id')::uuid;
      -- Get the application status before withdrawal
      select status into v_app_status
        from public.applications
        where crew_person_id = (p_payload->>'crew_person_id')::uuid
        and permanent_posting_id = v_posting_id
        and status in ('applied', 'shortlisted', 'selected');
      if v_app_status is null then
        raise notice 'No withdrawable application found for crew % posting %', p_payload->>'crew_person_id', v_posting_id;
        return;
      end if;
      ...
      update public.applications set status = 'withdrawn', updated_at = now()
        where crew_person_id = (p_payload->>'crew_person_id')::uuid
        and permanent_posting_id = v_posting_id
        and status = v_app_status;
      -- If was selected: close engagement and revert posting
      if v_app_status = 'selected' then
        update public.active_engagements set status = 'closed', outcome = 'withdrew'
          where permanent_posting_id = v_posting_id and status = 'active';
        update public.permanent_postings set status = 'active', updated_at = now()
    ...
    when 'PERMANENT.CANCELLED_BY_EMPLOYER' then
      v_posting_id := (p_payload->>'permanent_posting_id')::uuid;
      -- Check if in_negotiation (has selected candidate)
      perform 1 from public.permanent_postings where id = v_posting_id and status = 'in_negotiation';
      if found then
        -- Close engagement
        update public.active_engagements set status = 'closed', outcome = 'not_successful'
      ...
      -- Cancel posting
      update public.permanent_postings set status = 'cancelled', updated_at = now()
        where id = v_posting_id;
      -- Reject all pending/shortlisted
      update public.applications set status = 'rejected', updated_at = now()
        where permanent_posting_id = v_posting_id
        and status in ('applied', 'shortlisted');

    when 'PERMANENT.ENGAGEMENT_CLOSED' then
      -- Close engagement with outcome
      update public.active_engagements set
        status = 'closed',
        outcome = p_payload->>'outcome'
        where id = (p_payload->>'engagement_id')::uuid and status = 'active';
      -- If crew withdrew: revert posting to active
      if (p_payload->>'outcome') = 'withdrew' and (p_payload->>'closed_by') = 'crew' then
        -- Find posting from engagement
        select permanent_posting_id into v_posting_id
