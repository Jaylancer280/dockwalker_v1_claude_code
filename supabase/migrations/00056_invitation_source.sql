-- =============================================================================
-- Migration 00056: Application source column + auto-shortlist invited crew
--
-- Adds a `source` column to `applications` to track whether an application
-- originated from a direct apply or an accepted invitation. When source is
-- 'invitation', the application is created with status 'shortlisted' instead
-- of 'applied', so invited crew automatically appear in the Shortlisted tab.
--
-- Also fixes multi-crew invitation revocation: invitations are only revoked
-- when all positions are filled, not on every individual accept.
-- =============================================================================

-- 1. Add source column to applications
alter table public.applications add column source text
  check (source in ('direct', 'invitation'));

-- 2. Update apply_projection with new DAYWORK.APPLIED handler + ACCEPTED fix
...
begin
  case p_event_type
    ...
    when 'DAYWORK.APPLIED' then
      -- source = 'invitation' → auto-shortlist; otherwise normal 'applied'
