-- =============================================================================
-- Migration 00128: Relax REFERENCE.REQUESTED vessel-source gate
--
-- Drops the `source = 'curated'` requirement from the REFERENCE.REQUESTED
-- handler. While the vessel database is being populated/curated, all newly
-- created vessels enter the pending-curation queue (since 00119 + the
-- /api/vessels POST change). Forcing references to wait for admin approval
-- created a dead-end UX where users couldn't gather references on freshly
-- added vessels.
--
-- The other defensive gates remain unchanged:
--   • NDA vessels still rejected
--   • hidden_at vessels still rejected
--   • Vessel must still exist
--
-- Body is the 00126 apply_projection re-creation, line-for-line, with the
-- 3 source-check lines removed from the REFERENCE.REQUESTED handler.
