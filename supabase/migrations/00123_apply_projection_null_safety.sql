-- Apply_projection null-safety hardening (Risk D-2).
--
-- Three handlers run a `SELECT ... INTO v_filled, v_available FROM
-- public.dayworks WHERE id = ...` (or an `UPDATE ... RETURNING ... INTO`
-- equivalent) and then compare the variables in `IF v_filled >=
-- v_available THEN`. If the daywork row no longer exists (concurrent
-- admin deletion, future cleanup process, race with a cancellation),
-- the SELECT/UPDATE produces no row and the variables stay NULL.
-- `NULL >= NULL` evaluates to NULL, which an IF statement coerces to
-- FALSE — the handler proceeds, inserts an `applications.status =
-- 'accepted'` row, and creates an `active_engagements` row referencing
-- a daywork_id that doesn't exist (orphan FK in a table that is
-- supposed to be referentially clean).
--
-- Latent today (no code path deletes dayworks; we soft-cancel via
-- DAYWORK.CANCELLED_BY_EMPLOYER), but the failure mode is silent state
-- corruption that won't surface until a customer complains about a
-- ghosted job. This migration adds explicit guards in three sites:
--
--   1. DAYWORK.ACCEPTED        — line 367-378 in 00121
--   2. DAYWORK.INVITATION_ACCEPTED — line 392-403 in 00121
--   3. DAYWORK.POSITIONS_UPDATED  — line 349-355 in 00121 (UPDATE...RETURNING variant)
--
-- Each guard is a two-line addition: `if v_available is null then
-- raise exception '...'; end if;` immediately after the SELECT/UPDATE.
--
-- The function body below is otherwise an exact line-for-line copy of
-- 00121 — handler count must stay 71. Lessons-mandated apply_projection
-- replacement protocol: $$ count = 2, ends with `end case; end; $$;`,
-- every event type from 00121 carries through unchanged except for the
-- three handlers above.
--
-- Rollback emits a NOTICE pointing operators at re-applying 00121 to
-- restore the previous (un-guarded) projection body. The guards are
-- defensive only — un-applying them does not corrupt data.
...
begin
  case p_event_type
    ...
    when 'DAYWORK.POSITIONS_UPDATED' then
      ...
      -- D-2 null guard: if the daywork no longer exists, the UPDATE
      -- matches no rows and v_filled / v_available stay NULL. NULL >=
      -- NULL coerces to false in an IF, so the inner block silently
      -- no-ops — defensible behaviour, but better to surface the
      -- inconsistency loudly than to silently skip.
    ...
    when 'DAYWORK.ACCEPTED' then
      ...
      -- D-2 null guard: if the daywork no longer exists, the SELECT
      -- returns no rows and v_filled / v_available stay NULL. NULL >=
      -- NULL coerces to false, allowing the handler to insert an
      -- application + active_engagement with an orphan daywork_id FK.
      -- Raise instead of proceed.
      if v_available is null then
    ...
    when 'DAYWORK.INVITATION_ACCEPTED' then
      ...
      -- D-2 null guard: same orphan-FK risk as DAYWORK.ACCEPTED.
      if v_available is null then
