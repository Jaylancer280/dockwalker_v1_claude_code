-- =============================================================================
-- append_event: Writes an event AND updates projections in the same transaction
-- Called via Supabase RPC from the API layer
-- =============================================================================
...
-- =============================================================================
-- apply_projection: Routes events to the correct projection updater
-- =============================================================================
...
begin
  case p_event_type
    ...
    when 'DAYWORK.ACCEPTED' then
      ...
      -- Auto-supersede overlapping pending applications for this crew member
      update public.applications
    ...
    when 'AVAILABILITY.SET' then
      -- Upsert availability for each date in the range
    ...
    when 'MESSAGE.SENT' then
      ...
      -- Unknown event type — log but don't fail (forward compatibility)
...
-- =============================================================================
-- Command validation: prevent double-booking
-- Called before DAYWORK.ACCEPTED to check for overlapping engagements
-- =============================================================================
...
as $$
  select not exists (
    ...
    join public.dayworks d_new on d_new.id = p_daywork_id
    where ae.crew_person_id = p_crew_person_id
    and ae.status = 'active'
    and ae.start_date <= d_new.end_date
    and ae.end_date >= d_new.start_date
  );
$$;
