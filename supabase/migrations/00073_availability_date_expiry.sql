-- Migration 00073: Per-date availability expiry
-- Normal availability rows expire at end of their calendar day (date + 1 day).
-- Not-available rows keep 7-day TTL.
-- Past dates immediately expire after this backfill.

-- Step 1: Backfill existing normal availability rows
UPDATE public.availability_windows
SET expires_at = date::timestamp + interval '1 day'
WHERE not_available = false;

-- Step 2: Update apply_projection AVAILABILITY.SET handler
-- The normal path now uses d::date + interval '1 day' per row instead of client-sent expires_at.
-- Not-available path unchanged (uses client-sent expires_at for 7-day TTL).
--
-- We replace the full function to ensure consistency (per lessons: full body required).
-- Only the AVAILABILITY.SET handler changes (lines 352-364 equivalent).

-- NOTE: Full function replacement follows. Copied from 00072 with ONLY the
-- AVAILABILITY.SET normal path changed: expires_at = d::date + interval '1 day'
  ...
  case p_event_type
    ...
    when 'PROFILE.UPDATED' then
      update public.profiles set
        ...
        deck_name = case when p_payload ? 'deck_name' then p_payload->>'deck_name' else deck_name end,
