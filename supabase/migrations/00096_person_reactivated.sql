-- =============================================================================
-- Migration 00096: Add PERSON.REACTIVATED handler
--
-- A user whose persons row has deactivated_at set can restore their account
-- by completing a password reset within the retention window. The reset-password
-- flow appends PERSON.REACTIVATED, which clears deactivated_at via this handler.
-- The accompanying API route also lifts the auth ban (admin updateUserById).
--
-- This is the inverse of PERSON.DEACTIVATED. After 30 days, PERSON.DATA_SCRUBBED
-- erases PII and reactivation is no longer possible (the email account is gone).
-- =============================================================================
...
begin
  case p_event_type
    ...
    when 'PERSON.DATA_SCRUBBED' then
      -- Soft-delete engagement documents (cron handles storage cleanup)
      ...
      -- Delete notification channels (encrypted phone numbers — hard delete, zero retention)
      delete from public.notification_channels where person_id = p_person_id;
      -- Delete agent placement cities
      delete from public.agent_placement_cities where person_id = p_person_id;
      -- Replace PII in profile with anonymized placeholders
      ...
      -- Delete advisor conversations (messages cascade via ON DELETE CASCADE)
      delete from public.advisor_conversations where person_id = p_person_id;
      -- Scrub interaction logs (retain row structure for analytics, nullify PII)
    ...
    when 'AVAILABILITY.SET' then
      if coalesce((p_payload->>'not_available')::boolean, false) then
        ...
        -- Expire ALL existing windows (available + not_available) — full replacement
        update public.availability_windows set expires_at = now() where person_id = p_person_id and expires_at > now();
        insert into public.availability_windows (person_id, date, expires_at, city_id, port_id, not_available)
