-- =============================================================================
-- Migration 00099: Shore-based experience — structured entries for green crew
--
-- New tables:
--   - shore_experience_categories (30 categories, lookup)
--   - shore_experiences (person_id, category_id, employer_name, job_title, dates, description)
--
-- Extends:
--   - events.aggregate_type CHECK with 'shore_experience'
--   - apply_projection with SHORE_EXPERIENCE.ADDED/UPDATED/REMOVED (68 total)
--   - PERSON.DATA_SCRUBBED handler to delete shore_experiences
--   - admin_delete_person to delete shore_experiences
-- =============================================================================

-- 1. Shore experience categories lookup
create table public.shore_experience_categories (
  id uuid primary key default gen_random_uuid(),
  name text not null unique,
  sort_order int not null default 0
);

alter table public.shore_experience_categories enable row level security;

create policy "Anyone can read shore experience categories"
  on public.shore_experience_categories for select
  to authenticated
  using (true);

-- Seed categories
insert into public.shore_experience_categories (name, sort_order) values
  ('Hospitality & Hotels', 1),
  ('Food & Beverage / Culinary', 2),
  ('Medical & Healthcare', 3),
  ('Nursing & Caregiving', 4),
  ('Engineering & Technical', 5),
  ('Electrical & Electronics', 6),
  ('Construction & Trades', 7),
  ('Military & Armed Forces', 8),
  ('Security & Close Protection', 9),
  ('Law Enforcement & Emergency Services', 10),
  ('Fitness & Personal Training', 11),
  ('Beauty & Wellness / Spa', 12),
  ('Education & Tutoring', 13),
  ('Childcare & Nannying', 14),
  ('Events & Entertainment', 15),
  ('Music & Performing Arts', 16),
  ('Photography & Videography', 17),
  ('Retail & Customer Service', 18),
  ('Administration & Office Management', 19),
  ('Finance & Accounting', 20),
  ('IT & Technology', 21),
  ('Marine & Watersports (non-yacht)', 22),
  ('Diving & Underwater Services', 23),
  ('Agriculture & Outdoor', 24),
  ('Automotive & Mechanical', 25),
  ('Fashion & Textiles', 26),
  ('Interior Design & Décor', 27),
  ('Cleaning & Housekeeping', 28),
  ('Logistics & Supply Chain', 29),
  ('Travel & Tourism', 30);

-- 2. Shore experiences table
create table public.shore_experiences (
  id uuid primary key default gen_random_uuid(),
  person_id uuid not null references public.persons(id),
  category_id uuid not null references public.shore_experience_categories(id),
  employer_name text not null check (char_length(employer_name) <= 100),
...
create index idx_shore_experiences_person on public.shore_experiences(person_id);

alter table public.shore_experiences enable row level security;

create policy "Users can read own shore experiences"
  on public.shore_experiences for select
  to authenticated
  using (person_id = auth.uid());

create policy "Authenticated users can read any active person shore experiences"
  on public.shore_experiences for select
  to authenticated
  using (
    exists (
      select 1 from public.persons
      where persons.id = shore_experiences.person_id
      and persons.deactivated_at is null
    )
  );

-- 3. Extend aggregate_type CHECK
...
-- 4. Update admin_delete_person to include shore_experiences
...
-- 5. Update apply_projection (68 total: 65 from 00098 + 3 new shore experience handlers)
