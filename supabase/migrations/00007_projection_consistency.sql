-- =============================================================================
-- Projection consistency fixes
-- - Atomic onboarding
-- - Event-driven hat switching and message hiding
-- - Event-driven daywork completion side effects
-- - Sanitized vessel metadata for crew discovery
-- =============================================================================
...
as $$
  select
    ...
    v.vessel_type,
    v.size_band_id,
    vsb.label as size_band_label,
    v.nda_flag,
    v.owner_person_id
  from public.vessels v
  join public.vessel_size_bands vsb on vsb.id = v.size_band_id
  where v.id = p_vessel_id;
$$;

create or replace function public.onboard_person(
...
begin
  ...
  perform public.append_event(
    ...
    'person',
    p_current_hat,
    coalesce(p_profile, '{}'::jsonb) || jsonb_build_object('identity_type', p_identity_type),
    p_person_id
  );
end;
$$;

create or replace function public.clear_availability_dates(
  p_person_id uuid,
  p_dates date[]
)
returns void
