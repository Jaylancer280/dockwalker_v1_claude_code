-- =============================================================================
-- Migration 00132: Pre-launch audit P1 fixes (D-1, D-3, D-4, M-1, M-2, P-4)
--
-- Combines five P1 audit findings into one migration:
--
--   P1-D1 PERMANENT.INVITED state guard. Projection now matches the API-
--     layer status check at /api/permanent/[id]/invite/route.ts:85-90.
--     Without this, ledger replay against a re-seeded DB where the
--     posting later reached in_negotiation/cancelled would silently
--     create invitations against stale postings.
--
--   P1-D3 PERMANENT.SHORTLISTED projection tightened from
--     `status in ('applied', 'viewed')` to `status = 'applied'` only.
--     Permanent has no PERMANENT.VIEWED event — the 'viewed' clause
--     was dead code that diverged from the route-layer check.
--
--   P1-D4 permanent_invitations.permanent_posting_id FK changed from
--     `on delete cascade` to `on delete restrict`. Hard-deleting a
--     posting was previously cascading invitations away, which silently
--     nulled applications.invited_from_id (set null) — losing the
--     audit trail of how applications were sourced. Forces admin scripts
--     to soft-cancel postings instead of hard-deleting.
--
--   P1-M1 NDA IMO reveal mirrors the name-reveal pattern by adding
--     `OR ae.employer_person_id = auth.uid()` to both IMO branches.
--     Without this, an agent posting a daywork on a vessel they don't
--     own would see null IMO on their own posting once engaged.
--
--   P1-M2 NDA reveal status filter widened from `'active'` only to
--     `('active', 'completed', 'closed')`. Once an engagement
--     completes/closes, the crew member who genuinely worked on the
--     NDA vessel keeps access to its IMO + name (CLAUDE.md doesn't
--     walk back access at completion).
--
--   P1-P4 Index on persons(deactivated_at) where not null. The S-2
--     data-scrub cron runs daily with a `< cutoff AND deactivated_at
--     IS NOT NULL` filter — without an index, the scan is sequential.
-- =============================================================================

-- 1. P1-D4: change permanent_invitations.permanent_posting_id FK from
--    `on delete cascade` to `on delete restrict`. Forces soft-cancel
--    on postings instead of hard-delete.
alter table public.permanent_invitations
  drop constraint if exists permanent_invitations_permanent_posting_id_fkey;

alter table public.permanent_invitations
  add constraint permanent_invitations_permanent_posting_id_fkey
    foreign key (permanent_posting_id)
    references public.permanent_postings(id)
    on delete restrict;

-- 2. P1-P4: index for the S-2 data-scrub cron's nightly scan. Partial so
--    it only indexes the rows that need processing (< 1% of persons in
--    steady state).
create index if not exists idx_persons_deactivated_at
  on public.persons(deactivated_at)
  where deactivated_at is not null;

-- 3. P1-M1 + P1-M2: re-create get_vessel_public + get_vessels_public_batch
--    with the IMO employer-side reveal and the widened status filter.

-- =============================================================================
-- get_vessel_public + get_vessels_public_batch — audit P1-M1 + P1-M2 fixes.
--
-- P1-M1: IMO reveal previously matched only `ae.crew_person_id = auth.uid()`,
--   so an agent posting a daywork on a vessel they don't own would see null
--   IMO on their own posting once engaged. Mirror the name-reveal pattern
--   by adding `OR ae.employer_person_id = auth.uid()` to both IMO branches.
--
-- P1-M2: All four reveal branches (2 IMO + 2 name) and the row-visibility
--   filter previously gated on `ae.status = 'active'`. Once a daywork
--   transitions to 'completed' or a permanent engagement to 'closed',
--   the crew member who genuinely worked on the NDA vessel loses access
--   to its IMO + name — including in their own historical experience
--   records. CLAUDE.md doesn't walk back access at completion. Widen
--   the status filter to ('active', 'completed', 'closed') so an
--   engagement that existed retains the reveal.
-- =============================================================================
    ...
    case
      ...
      when exists (
        ...
        where d.vessel_id = v.id
          ...
          and ae.status in ('active', 'completed', 'closed')
      ) then v.imo_number
      when exists (
        select 1 from public.active_engagements ae
...
-- 4. P1-D1 + P1-D3: re-create apply_projection with the PERMANENT.INVITED
--    state guard and the PERMANENT.SHORTLISTED status tightening. The
--    function body is otherwise an exact copy of 00131's; the only diffs
--    are the two handlers above.
