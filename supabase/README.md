...
| Locations V1 marina import                    | 00104     | OSM-sourced marina dataset (5,957 records post-filter + hub-normalization). Renames 7 launch regions to `_legacy_*` to avoid unique-name collision, upserts 63 country-based regions (UUIDv5), re-parents 29 curated cities, deletes legacy regions, inserts 3,336 new cities, 4 curated ports enriched with OSM lat/lon/metadata, 5,933 new ports.                                                        |
